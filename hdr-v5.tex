...
%nerveux en général. 
%%selon la double perspective du traitement et
...
en neurosciences computationnelles, apprentissage automatique et contrôle moteur. Ils s'inscrivent dans une perspective neuromimétique 
visant à mieux comprendre la logique à l'œuvre au sein du système
nerveux des êtres vivants.
%Conjointement avec les neurosciences, %des neurosciences invite par ailleurs à constamment réviser nos modèles et hypothèses sur ces questions.
%Ce ``travail en progression'' 
%cette approche 
%vise à 
Le but de cette recherche est d'améliorer la connaissance du cerveau et des pathologies nerveuses, mais
également de repenser la manière de concevoir des objets communicants (robots mobiles, interfaces vocales, jeux, etc.), 
afin qu'ils interagissant de manière plus ``naturelle'' avec leur environnement.
 
%Ces travaux reposent, comme nous le verrons, sur un certain nombre de choix méthodologiques~:
 Les modèles étudiés sont des systèmes dynamiques de grande dimension, 
	définis par un graphe d'interactions entre des unités de calcul élémentaires. 
	\begin{itemize}
		\item Nous étudions les principes du calcul ``collectif'' 
		dans ces architectures, tels que le ``temps de calcul'' corresponde au temps 
		de relaxation vers un \textit{attracteur}. % Ce calcul est réalisé par l'ensemble des neurones du réseau. 
		Les comportements d'ensemble  sont alors interprétés comme des implémentations d'opérateurs  plus simples.
		\item Nous montrons en particulier que l'utilisation d'un graphe de connexions aléatoire permet, 
		en régime critique, de %constituer un ensemble  varié de  réponses à des signaux extérieurs. 
		%L'aléa des connexions permet donc de 
		structurer l'espace des entrées sensorielles
		en régions distinctes, %autrement dit ``digitalise'' ses entrées, 
		selon un principe proche de l'{``auto-encodage''}.
	\end{itemize}
	
    
	L'apprentissage automatique est abordé dans une perspective neuro-réaliste. 
	\begin{itemize}
		\item Nous prenons en compte les contraintes biologiques, comme la contrainte de localité de Hebb, 
		le caractère récurrent du graphe de connexions, ou encore le caractère 
		discret des signaux émis par les neurones, pour développer nos modèles.
		\item Dans ce cadre, 
		nous montrons que la reconnaissance de formes fonctionne selon le principe 
		de la \textit{rétroaction positive} (ou résonance) entre un signal présenté en entrée  
		et une des ``formes'' possibles que peut prendre l'activité du réseau.
	\end{itemize}
	 
	
	Toujours dans une perspective de réalisme, nous cherchons à prendre en compte 
	les causalités circulaires existant entre un contrôleur neuromimétique et son environnement, 
	sur des tâches de contrôle moteur
	dites ``en boucle fermée''.
	\begin{itemize}
		\item Nous cherchons  à étendre 
		certains principes utilisés pour l'apprentissage de formes, 
		en particulier lorsque l'apprentissage est guidé par un signal de renforcement. 
		\item Dans ce cadre, nous avons contribué à établir une correspondance entre la règle de Hebb, 
		dans sa variante ``guidée'', et la recherche du maximum de vraisemblance dans un espace de couples 
		(perception, action), où la combinaison de recherches d'optima locaux s'apparente à la 
		recherche d'un optimum global en l'absence de modèle de l'environnement.
	\end{itemize} 
...
qui freinent l'apprentissage de lois de contrôle dans un espace d'actions continu.
En particulier, si l'on souhaite construire des dispositifs artificiels capable d'apprendre des tâches motrices de la même manière qu'ils apprennent à classifier des signaux et des images,
...
par opposition au  ``contrôle du déplacement'' utilisé en théorie du contrôle classique. Différents projets, visant à développer et appliquer ces principes, sont proposés dans le cadre de ce mémoire.
...
Entre les deux,  un niveau architectural (dit ``mésoscopique'') doit également être considéré, constitué de circuits imbriqués intégrant l'activité de larges portion du cerveau.
...
Plusieurs revues de neurosciences, comme Neuroimage ou J. Neurosciences, acceptent également des papiers du domaine.
...
\begin{itemize}
	...
	\item  Enfin, le chapitre 5 met en perspective ces différents résultats, et développe plusieurs pistes d'études pouvant servir de base à des travaux futurs.
\end{itemize}
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		l'ensemble des états de sortie des neurones depuis l'instant initial, i.e.  $\boldsymbol{S}(t) = \{s_i(t')\}_{i \in \{1,...,N\}, t' \in [t_0, t[}$. 
...
base d'exemples, via des mécanismes d'{``auto-encodage''}. L'opération algorithmique réalisée par le pattern matching est alors graduelle : degré d'appartenance
...
La mesure de la dimension effective permet donc de distinguer des signaux ``stabilisateurs'', tendant à corréler les neurones, des signaux déstabilisateurs, tendant à décorréler les neurones.
...
tel que~:
\begin{itemize}
	\item La réduction signe l'appariement avec une des formes stockées dans le réseau, et désigne les signaux connus.
	\item L'absence de réduction signe l'absence d'appariement avec les formes stockées dans le réseau, et désigne les signaux inconnus.  
\end{itemize}

\paragraph{}Ce type d'appariement dit conditionnel (avec rejet) autorise le réseau à \textit{ne pas nécessairement choisir} parmi l'ensemble des choix possibles. Autrement dit, le réseau peut produire la réponse ``je ne sais pas'' lorsque le signal présenté n'est pas suffisamment proche d'une forme connue.
...
de sélection dans lequel \textit{des activités corrélées tendent à se connecter, et des activité décorrélées à se déconnecter}. 
Ce modèle de plasticité synaptique, dite ``potentiation à long terme'' (Long Term Potentiation - LTP). \shortcite{Heb49}, a été confirmée à de nombreuses reprises par les observations \shortcite{Bli73,BIPOO98}. 
...
\paragraph{Plasticité induite} Dans le cadre du contrôle moteur, l'apprentissage vise à produire des régularités dans cette dynamique afin de stabiliser certaines réponses motrices.
...
\paragraph{Champ moyen}
 ...
 Un paramètre d'ordre peut être vue comme une \textit{unité de description} de l'activité d'un système dynamique, généralement de grande taille.
...
c'est à dire reposant sur des modèles de l'environnement physique. L'apprentissage, en ce sens, se justifie lorsque, d'une manière ou d'une autre, les modèles ne sont pas considérés satisfaisants et doivent être ``améliorés''. 
...
\paragraph{}
On oppose généralement au contrôle par feedback négatif (égalementt appelé \textit{poursuite}) le contrôle dit \textit{balistique} dans lequel l'erreur motrice n'est pas mesurée. Le contrôle repose sur la consigne $\boldsymbol{x}^*$ seule. On parle aussi de contrôle en position. 
...
\paragraph{}
Les travaux présentés dans ce mémoire offrent des portes d'entrée vers le principe de contrôle par appariement et feedback positif
...
Les réponses produites par la STDP sur des entrées statiques s'apparentent à des ``bursts'' (bouffées) d'activité, qui impliquent 
...
\subsubsection{j. Dans des espaces d'état et d'action continus, l'apprentissage par renforcement peut être implémenté via la plasticité Hebbienne induite et le principe de trace synaptique. La dynamique endogène joue le rôle d'un bruit d'exploration.}
...
\paragraph{}Par ailleurs, ces algorithmes reposent  sur des modèles paramétriques de la mesure, qui permettent la mise à jour du modèle après chaque série de mesures, selon le principe du maximum de vraisemblance, implémenté via une descente de gradient ou via l'algorithme EM (voir page \pageref{eq:Friston}). 
 \begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		\item Nous montrons qu'avec une décision non-stochastique et en l'absence de modèle, le classifieur converge vers taux de réponse comparable à l'état de l'art en classification BCI.
...
Le mécanisme de l'appariement, au centre des algorithmes d'apprentissage, prend la forme d'une réduction de complexité de la dynamique intrinsèque (étude {\bf f}). Deux grandes familles  plasticités sont abordées~:
\begin{itemize}
	\item Les plasticités d'{``ordre 0''}, guidées par un signal de renforcement, identifient les relations d'entrée-sortie les plus favorables, selon un principe qui s'apparente gradient logistique (études {\bf j} et {\bf l}).
...
\begin{itemize}
\item Dans un réseau récurrent, le caractère hétérogène et irrégulier du graphe de con\-ne\-xions permet, en régime critique, de constituer un ensemble  varié de  réponses à des signaux extérieurs. Le hasard (l'aléa des connexions) constitue ici un principe structurant qui sépare l'environnement en régions distinctes, autrement dit qui ``digitalise'' l'environnement, selon un principe proche de l'{``auto-encodage''} tel qu'il est mis en œuvre dans les réseaux profonds \shortcite{Hinton2006} (études {\bf a}, {\bf b}, {\bf c} et {\bf d}).  
...
Il m'a permis de mettre à jour ce qui semble être une difficulté fondamentale dans l'apprentissage de compétences motrices, qui nécessite probablement un examen fouillé dépassant le cadre de ce rapport.
...
Il semble y avoir une séparation conceptuelle et pratique entre, d'une part, l'approche systèmes dynamiques et, d'autre part, la reconnaissance de formes. 
Cette difficulté semble être liée au fait que les distances, telles qu'elles sont définies dans l'espace euclidien de la commande, ne sont pas conservées dans les espaces de redescription. 
%La multitude des récepteurs sensoriels, d'une part, et le caractère continu des déplacements, d'autre part, forment un tout disparate. 
...
\subsubsection{Problème de l'assemblage}
Les modèles classiques de la commande motrice reposent implicitement sur l'hypothèse d'un espace physique extensif où prennent place des transformations linéaires\footnote{
	Ces prédictions s'apparentent à des opérations géométriques (isométriques), soit des translations et
...
Par exemple, un simple ``pattern matching'' nécessiterait un apprentissage par cœur de la conséquence visuelle 
de toutes les combinaisons de dispositions d'objets et de tous les changements de référentiel tronc-tête, ce qui parait 
computationnellement très coûteux (\shortcite{Pouget1997,Pouget2002}). Il est nécessaire néanmoins que le ``lien'' praxo-visuel entre les différents objets de la scène ne soit pas perdu
...
Le mécanisme de construction de l'espace métrique environnant reste encore à l'heure actuelle non-élucidé. Nous avons vu que le caractère ``digital'' de l'activité neuronale rend difficile, en interne, la construction d'opérateurs sur cet espace.
Ce mécanisme semble pourtant pouvoir être implémenté sur la base de principes d'appariement très simples, mais n'a, à ma connaissance, pas encore été proposée en modélisation. Deux principes semblent s'imposer dans ce cadre : 
\begin{enumerate}
...
\begin{itemize}
	\item les commandes motrices balistiques correspondent à la définition d'un attracteur dans l'espace de la tâche.  Cet attracteur est produit comme combinaison de différents champs de vecteurs (appelés ``primitives motrices''). Ce type de commande ne nécessite pas référence à un point d'opération externe (voir aussi page \pageref{sec:feed-plus}). La dynamique de relaxation et les constantes de temps propres à l'effecteur contrôlé conduisent les différentes articulations vers leur point d'équilibre.
	\item L'ensemble des points que le bras peut atteindre forme un espace tridimensionnel. Le cortex moteur reproduit cette organisation spatiale~: le cortex est organisé sous la forme d'une carte motrice bidimensionelle où chaque région de la carte correspond à une combinaison différente de primitives motrices conduisant la main vers un point de l'espace tridimensionel environnant (péripersonnel). 
\end{itemize}
Ainsi:
	\begin{itemize}
...
(voir equations. (\ref{eq:SIG}) (\ref{eq:SD})).
...
l'existence de modes et de résonances extrêmement variés lorsque le signal est applique sur un seul nœud du réseau (micro-stimulation). 
...
La question adressée par le modèle publié en 2004 \shortcite{Dau04} est celle de l'implémentation d'un modèle de champ neuronal (voir pages \pageref{sec:NField1} et \pageref{sec:NField}) sur un support composé
d'unités discrètes.
%, comme l'activité dite ``persistante'' ainsi qu'une bonne sensibilité au signal d'entrée (switch). 
Sur les modèles jusqu'alors proposés, dans un cas l'activité focale était soit uniquement réactive \shortcite{Han96},  
dans l'autre cas,
...
\begin{figure}[t!]
	...
	\caption{Input presentation in a topologically organized network (only the excitatory layer owns a topology). The network is composed of $N^{(1)} = 1000$ neurons and $N^{(2)} = 300$ neurons. %The effective sparsity are ρ (11) = 2.6%, ρ (12) = 22.5%, ρ (21) = 7.6%,
	%	(22)
	%	= 22%. 
	The excitatory links are globally faster ( $\simeq 5$ time steps) than inhibitory links ($\simeq 9$ time steps). Four inputs (red) are sent. The first input is sent at time 301–340 on neurons
		591–600. The second input is sent at time 501–520 on neurons 181–200. The third input is
		sent at time 701–740 on neurons 691–700. The last input crosses the whole field, activating 10
		neurons at each time step. {\bf Upper left~:} global activity of the excitatory layer. {\bf Lower left~:} mean
		activity on excitatory layer. {\bf Middle right~:} global activity of the inhibitory layer. {\bf Lower right~:}
		mean activity on inhibitory layer. {\bf Upper right insets~:} distribution of the sum of incoming
		weights from excitatory and inhibitory populations, for 1000 excitatory neurons (left) and 300
		inhibitory neurons (right) (from Daucé, 2004).}
	\label{fig:nc-field}
\end{figure} 
...
en changeant les caractéristiques du seuil d'activation afin de contrôler le niveau d'activation moyen.
Ces travaux ont permis de mettre en évidence un nombre très important d'attracteurs dans certaines gammes de paramètres.
...
\end{center}
\caption{
	{\bf Cores of the 8 principal clusters, on a SL attractor set}. The initial attractors set contains 28,849 attractors, with $P =1$ and $G=900$. 
	The clusters are obtained by a double-pass clustering, with inclusion match similarity ($k = 0.8$). The cores are defined as the 10\% most active nodes in a cluster. {\bf a:} core of cluster 1 (4,089 attractors);  {\bf b:} core of cluster 2 (3,048 attractors);  {\bf c:} core of cluster 3 (2,045 attractors);  {\bf d:} core of cluster 4 (1,573 attractors);  {\bf e:} core of cluster 5 (907 attractors);  {\bf f:} core of cluster 6 (892 attractors);  {\bf g:} core of cluster 7 (881 attractors);  {\bf h:} core of cluster 8 (844 attractors) (from Golos et al, \textit{submitted}).  
}
\label{fig:PLOS}
\end{figure}
...
Ainsi les régularités observées durant les premières phases de l'apprentisage ne sont plus nécessairement présentes
...
\begin{itemize}
	\item L'apprentissage ``en ligne'' \shortcite{leCun2004}
...
du passé sont prises en compte avant d'être oubliées \shortcite{Dauce2015}.
