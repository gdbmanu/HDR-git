...
Contrairement aux classifieurs traditionnels qui apparient \textit{nécessairement} le signal d'entrée avec une des formes stockées dans la structure, la séparation de l'espace des signaux entre signaux réducteurs et signaux non-réducteurs s'apparente à un opérateur  d'\textit{appariement conditionnel} tel que~:
\begin{itemize}
	\item les signaux de forme connue s'apparient avec une des formes stockées dans le réseau;
	\item les signaux inconnus ne s'apparient avec aucune des formes stockées dans le réseau et sont \textit{rejetés}.  
\end{itemize}

\paragraph{}Autrement dit l'appariement conditionnel (avec rejet) autorise le réseau à \textit{ne pas choisir}. Parmi l'ensemble des réponses possibles, le réseau produit la réponse ``je ne sais pas'' lorsque le signal présenté n'est pas suffisamment proche d'une forme connue.
...
%Le problème des disparités d'échelle est souvent abordé à l'aide de descripteurs macroscopiques (dites variables de champ moyen) servant à décrire de manière concise l'activité de grandes populations  de neurones.
	...
	Les modèle de champ moyen considèrent l'activité instantanée de populations de neurones à travers un ou plusieurs paramètres d'ordre, également appelées variables macroscopiques, qui expriment une caractéristique d'ensemble de l'activité qui se développe dans le réseau. 
...
{\color{Gray}
	...
	De nombreux modèles stochastiques de neurones ont été proposés qui prennent en compte le caractère stochastique de la 
...
{\color{Cyan}
[il manque des refs sur le sampling temporel et les intégrateurs neuronaux  \shortcite{Gold2001,Simen2011,khoei2013}
...
\end{itemize}
Ces algorithmes reposent par ailleurs sur des modèles paramétriques de la mesure, qui permettent la mise à jour du modèle après chaque série de mesures, selon le principe du maximum de vraisemblance, implémenté via une descente de gradient ou via l'algorithme EM (voir page \pageref{eq:Friston}). 
 \begin{itemize}
...
\subsubsection{k. Commande balistique et intégratin sous-liminaires}

L'étude des mouvements d'orientation visuelle constitue un champ de recherche à part entière en neurosciences.

poursuite vs. commande balistique.

Rôle des intégrateurs neuronaux

Mécanisme de décision par ``levée d'inhibition'' (niveau ellulaire etnon au niveau de population)
