...
Dans le cas des modèles de filtrage, il s'agit par exemple d'améliorer les modèles $P$ (processus externe) et $Q$ (mesure), afin de mieux prendre en compte les facteurs exogènes non modélisés afin de mieux prédire les entrées.  %L'apprentissage consiste ainsi à 
...
Cette famille de modèles est basé sur un modèle \textit{génératif} interne $P_\Theta(\boldsymbol{X}|\boldsymbol{x},\boldsymbol{u})$ et $Q_\Theta(\mathcal{I}|\boldsymbol{x})$, où $\Theta$ est le jeu de paramètres définissant les deux distributions.  Le vecteur d'état $\boldsymbol{x}$ est ici produit selon un principe de ``codage par population'' \shortcite{Georgopoulos1986,Averbeck2006}, autrement dit via un échantillonnage de $P_\Theta$. 
