...
se développent depuis une quarantaine d'années, visent à mieux comprendre la logique à l'œuvre dans les circuits neuronaux, selon la double perspective du traitement et
de la transformation des signaux sensoriels, d'une part, et du contrôle et de la coordination des actions, d'autre part. 
Il s'agit, au niveau microscopique, de mieux comprendre le ``code'' des signaux émis par les cellules nerveuses, mais également, au niveau ``macroscopique'', de mieux comprendre les principes organisationnels 
permettant à ce réseau complexe qu'est le cerveau de contrôler l'appareil sensoriel et moteur. 
Le développement rapide des neurosciences invite par ailleurs à constamment réviser nos modèles et hypothèses sur ces questions.
Ce ``travail en progression'' devrait conduire à terme à améliorer le diagnostic et la réhabilitation de certaines pathologies nerveuses, mais également à changer la manière de concevoir des objets communicants (robots mobiles, interfaces vocales, jeux, etc.), afin qu'ils interagissant de manière plus ``naturelle'' avec leur utilisateur.

\paragraph{}
Plus concrètement, ce rapport présente mes contributions à la recherche 
en neurosciences computationnelles, apprentissage automatique et contrôle moteur. Les résultats s'appuient en grande partie sur les comportements de systèmes dynamiques de grande dimension, c'est  dire constitués par l'interaction de nombreuses unités couplées.
Ces travaux reposent, comme nous le verrons, sur un certain nombre de choix méthodologiques~:
\begin{itemize}
	\item Aborder l'apprentissage automatique dans une perspective neuro-réaliste. Prendre en compte les contraintes biologiques, comme la contrainte de localité de Hebb, le caractère récurrent et hétérogène du graphe de connexions, ou encore le caractère discret des signaux émis par les neurones, pour développer nos modèles.
	\item Étudier les principes du calcul ``collectif'' dans les architectures neuronales récurrentes, de sorte que le ``temps de calcul'' corresponde au temps de relaxation vers un \textit{attracteur}. % Ce calcul est réalisé par l'ensemble des neurones du réseau. 
	Les comportements d'ensemble  sont alors interprétés comme des \textit{implémentations} d'opérateurs  plus simples.
	\item  Étendre certains principes liés à l'apprentissage de formes  au cas du contrôle moteur, en particulier dans le cadre du contrôle dit ``en boucle fermée'', et du guidage de l'apprentissage par un signal de renforcement. 
\end{itemize}

\paragraph{}

Dans les architectures neuronales définies par un graphe de connexions récurrent, nous montrons que la reconnaissance de formes fonctionne selon le principe de la \textit{rétroaction positive} (ou résonance) entre un signal présenté en entrée  et une des ``formes'' possibles que peut prendre l'activité du réseau.
Ce principes semble pouvoir s'étendre au cas de l'apprentissage moteur.
Néanmoins, le travail de relecture et de compilation occasionné par la rédaction de ce rapport a mis en lumière certaines difficultés fondamentales liées à la modélisation de l'apprentissage de compétences motrices, qui freinent la possibilité d'émergence d'une loi de contrôle dans un espace d'actions continu.
Ce rapport propose donc quelques pistes de recherche pour y remédier, en proposant un contrôle moteur fondé sur la rétroaction \textit{positive}.
