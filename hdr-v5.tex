...
\paragraph{Alternance comportementale}\label{page:alternance}
...
Cette expérience robotique offre un exemple clair de couplage entre activité mécanique (déplacements moteurs) et l'activité 
...
\subsubsection{i. Dans des espaces d'état et d'action continus, l'apprentissage par renforcement peut être implémenté via la plasticité Hebbienne induite et le principe de trace synaptique. La dynamique endogène chaotique joue le rôle d'un bruit d'exploration.}
...
Dans un contexte de réseau de neurones récurrents aléatoires à dynamique intrinsèque chaotique, dont le caractère irrégulier sert de support à l'exploration 
...
Dans un contexte de réseau de neurones récurrent,
le but de la plasticité est de favoriser les réponses liées à un renforcement positif (sens ``Hebbien''), et de défavoriser les réponses liées à un renforcement négatif (sens
anti-Hebbien).
...
\begin{itemize}
	...
	\item Les modèles proposés dans \cite{Dau09} (page~\pageref{page:ICANN09} et annexe \ref{app:ICANN09}) et \cite{Dauce2010}, (page~\pageref{page:NCO10} et annexe \ref{app:NCO10}) utilisent les mêmes principes
...
\subsubsection{z. L'appariement par ``accumulation d'évidence'' implémente, de manière à la fois pratique et biologiquement plausible, un processus de décision dans un espace de choix concurrents.}

Les modèles probabilistes basés sur des processus stochastiques markoviens (voir equations (\ref{eq:accum1}) - (\ref{eq:accum1})) sont très populaires en modélisation biologiquement inspirée du fait de leur capacité rendre compte des profils de réponse extrêmement bruités à la fois de l'activité neuronale et des réponses comportementales.
Les processus markoviens peuvent également implémenter, au même titre que les modèles déterministes, les principes d'activité persistante, de rémanence et de mémoire à court terme.
