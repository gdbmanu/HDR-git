...
Nous considérons ici le cadre un peu plus restreint où le modèle d'état  $P$ n'est pas connu. 
...
Nous considérons le cas où l'\textit{observation} $\boldsymbol{I} = (\boldsymbol{I}(1),...,\boldsymbol{I}(t)))$ est 
...
Dans le cadre des modèles de l'appariement, l'évolution temporelle de  $\hat{f}$ constitue un modèle de la décision par délibération, où, comme dans le cas des réseaux de neurones récurrents, plusieurs ``passages'' sont nécessaires pour aboutir à un compromis. L'estimation faite à l'instant $t$ sert de prior pour affiner l'estimation à l'instant $t+1$. 

{\color{Cyan}
\begin{itemize}
	\item L'apprentissage par renforcement consiste à construire un modèle génératif $\pi_\Theta(\boldsymbol{u}|\boldsymbol{I})$ basé sur
	l'appariement entre  permettant, via la réalisation d'une action, de générer la nouvelle observation. La ``vraisemblance'' 
	se situe  L'appariement consiste ici à associer une observation $\boldsymbol{I}$ à des  consignes motrices $\boldsymbol{u}$ (assimilables à des états latents), via une `politique'' , qui joue formellement le rôle d'une probabilité a posteriori sur l'espace des actions. L'algorithme du ``policy gradient'' \cite{Wil92,peters08} construit la fonction d'appariement par ``apprentissage direct'', c'est à dire principalement un e descente de gradient dans    Le cas des  et le signal de renforcement sert à construire la et la fonction $\hat{f}$ 
\end{itemize}

Dans les modèles sans dynamique d'état (i.e. $P_\Theta=\text{Id}$), le mécanisme de filtrage vise à affiner au cours du temps différentes hypothèses concurrentes concernant l'état du monde, via une séquence d'observations. 
Dans le cas de signaux d'entrée de faible intensité o fortement bruités, le passage du temps permet en effet d'éliminer certaines hypothèses par 

principe de révision des croyances, 
...
Dans les environnements bruités, un même 
Dans ce cadre, le mécanise de la \textit{décision} consiste à 
