...
Il peut s'agir des contraintes physiques (limites d'extension, de course, ...) mais
...
Ce modèle de plasticité synaptique, dite ``potentiation à long terme'' (Long Term Potentiation - LTP), a été confirmée à de nombreuses reprises par les observations \shortcite{Bli73,BIPOO98}. 
...
Le mécanisme fondamental sur lequel s'appuient la plupart des algorithmes d'apprentissage automatique est celui de l'\textit{appariement} (``\textit{pattern matching}'') entre un \textit{dictionnaires de formes} et un vecteur (ou un ensemble de vecteurs) d'entrée. 
...
distinctes dans le spectre de Fourier du signal moyen (voir section \ref{sec:criticalite}, page \pageref{sec:criticalite}). La différence qualitative entre les différents régimes observés (différence de fréquence principale) peut servir de support à une catégorisation des l'espace des entrées.  
...
(voir section \ref{sec:multi_pop}, page \pageref{sec:multi_pop}).
...
Le papier qui présente ces idées de la manière la plus complète est celui de Neural Networks \shortcite{Dau98A} (voir annexe \ref{app:NN98}).
...
$N^{(1)}=1600$ et $N^{(2)}=200$}. 
Le système est continuellement simulé par un patron spatio-temporel périodique de période 6
(représentant un saut de grenouille).
{\bf a.} Activité neuronale sur la couche secondaire. 30 signaux individuels (sur 200)
sont représentés. La figure inférieure donne l'activité globale moyenne.
...
d'entrée sont présentées en niveaux de gris sous forme de vignettes 40 x 40, 
...
\begin{figure}[b!]
	...
	\caption{\footnotesize{\textbf{a}. 
			Estimation du nombre de degrés de libertés, pr pas de 100 ms, sur 25 secondes (25000 ms) de dynamique. La STDP est appliquée après 5 secondes. \textbf{b}. 
...
\caption{\footnotesize{Activité de la population excitatrice durant la STDP. -A- Entrée non-périodique -B- Entrée périodique (extrait de Daucé, 2014).}}
\label{fig:ESANN14}
\end{figure}
...
\caption{ \footnotesize{{\bf Dispositif expérimental}. $N$ patrons spatio-temporels sont présentés au réseau afin d'être classés en $K$ catégories. Le réseau est composé de 3 populations. La couche d'entrée est composée de 4 neurones (étiquetés A, B, C et D).
Les connexions d'entrée suivent une loi Gaussienne de moyenne 0 et d'écart-type 0,04. 		 
La couche cachée contient 100 neurones complètement connectés. Les poids synaptiques de la couche récurrente suivent une loi Gaussienne de moyenne 0 et d'écart-type 0,02. La couche de sortie est composée de $K$ neurones, avec des connexions latérales inhibitrices (non représentées). Les poids de sortie suivent une loi Gaussienne de moyenne 0,09 et d'écart-type 0,01
...
($+30^\circ$,$+60^\circ$,$+90^\circ$). La robot est placé dasn un environnement ouvert (ce n'est pas une simulation). L'association entre un jeu d'amers visuels (points de contraste élevé, notés par un 'x' sur la figure) combinés avec leur position angulaire constitue l'entrée visuelle. Après trois mouvements, le robot a fait un demi-tour (la répétition de ces 3 mouvements ramène le robot sur sa position initiale).
{\bf b:}  Example de recalage moteur suite à un décalage visuel. Les deux premiers pas de temps correspondent à la dynamique transitoire. Les mouvements suivants correspondent à la séquence périodique apprise $(+30^\circ,
+60^\circ, +90^\circ)$, accompagnée de la séquence visuelle apprise (de sorte que vision et mouvement sont dynamiquement associés). En raison de frottements au niveau du sol, le mouvement réel, ainsi que la scène visuelle, se décalent, conduisant à une dissociation progressive de 
la scène visuelle et du mouvement. Un changement de comportement est observé à $t=25$, correspondant au décrochage entre le flux visuel et les mouvements associés. Finalement, après de nouveaux transitoires, le robot trouve à nouveau une bonne correspondance visuelle et réitère sa séquence périodique. {\bf c:} Après que le robot a atteint son comportement périodique, l'entrée visuelle est masquée. L'absence d'information visuelle conduit rapidement le robot vers un comportement ``chaotique'' (d'après Dauce et al., 2002).}
	}
	\label{fig:PerAc-2}
\end{figure} 
