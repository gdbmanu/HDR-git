...
\begin{itemize}
 ...
 permettant de développer des dynamiques méta-stables à ``foyers d'activité''  \shortcite{dauce04a} (voir section \ref{sec:NatComp} et annexe \ref{app:NC04})
...
attracteurs se développant au sein d'un tel réseau (voir section \ref{sec:PLOSsub} et annexe \ref{app:PLOSsub}).
...
{\color{Cyan}
Ce document est organisé autour de trois pôles~: 
\begin{itemize}
...
\part{\color{Orange}Principaux résultats et perspectives}
...
\begin{itemize}
	\item 
	Les résultats présentés dans ce rapport s'appuient sur les études des comportements à large échelle de systèmes dynamiques constitués par l'interaction de nombreuses unités couplées, qui lient 
	l'activité ``microscopique'' à des grandeurs ``macroscopiques'' basées sur les comportements moyens et les corrélations d'activité \shortcite{Wilson1972,Ama72,Amit1985,Som88,Ces94B,Ami97}, etc.
...
Dans le cadre de ce rapport, les résultats  ont été organisés autour de trois grandes thématiques~:
\begin{enumerate}
	...
	\item La thématique du contrôle moteur et de l'apprentissage moteur dans des espaces continus, selon le principe de l'apprentissage par renforcement principalement.
\end{enumerate}


Enfin, le travail de compilation et de relecture/réexamen de résultats parfois anciens liés à la rédaction de ce rapport a été l'occasion de développer quelques rapprochements qui ne sont pas présentes en tant que telles dans les publications. 
Je cherche en particulier à éclairer les résultats
sous l'angle de l'opération d'\textit{appariement} (``pattern matching''), qui est au centre des algorithmes d'apprentissage automatique, en particulier~:
\begin{itemize}
...
\end{align}
avec $\boldsymbol{S}(t)$ l'activité développée dans le réseau jusqu'au temps $t$, $\boldsymbol{I}(t)$ le signal d'entrée et $G$ le graphe (voir section \ref{sec:activite} pour plus de détails).
On dit également 
...
La fonction de réponse $f$ est en général une projection sur un espace $\mathcal{Y}$ de dimension plus petite que l'espace d'entrée $\mathcal{X}$.
...
\paragraph{Perspective computationnelle de la plasticité de Hebb}
...
\end{itemize} 
Ce cadre général est celui des réseaux de neurones~: la fonction de réponse est implémentée sous la forme d'un graphe : les noeuds sont les ``neurones'' et les arêtes sont les ``synapses''. 
...
\paragraph{}Nous aurons l'occasion d'étudier par la suite les modèles plus directement inspirés par la biologie. Avant cela, nous abordons l'autre grande approche utilisée en modélisation neuronale, à savoir les modèles issus de la théorie des systèmes dynamiques et du contrôle.
...
\subsection{Espace métrique et espace sensoriel}	
...
Cette famille de modèles est basé sur un modèle \textit{génératif}  $P_\Theta(\boldsymbol{X}|\boldsymbol{x},\boldsymbol{u})$ et $Q_\Theta(\mathcal{I}|\boldsymbol{x})$, où $\Theta$ est le jeu de paramètres définissant les deux distributions.  
Dans le cas des modèles de filtrage, il s'agit par exemple d'améliorer les modèles $P_\Theta$ (processus externe) et $Q_\Theta$ (mesure), afin de mieux prendre en compte les facteurs exogènes non modélisés afin de mieux prédire les entrées.  %L'apprentissage consiste ainsi à 
...
\begin{align}
\max_{\Theta'} \int \mathbb{P}(\Theta'|\boldsymbol{x},\boldsymbol{I},\boldsymbol{u}) \hat{f}(\boldsymbol{x}) d\boldsymbol{x} \label{eq:Friston}
\end{align}
...
$$\max_{\Theta',\boldsymbol{u}'}\int \mathbb{P}(\Theta',\boldsymbol{u}'|\boldsymbol{x},\boldsymbol{I}) \hat{f}(\boldsymbol{x}) d\boldsymbol{x} $$

%{\color{Cyan} Dans le cas des modèles du cervelet $I$ est la conséquence proprioceptive de la commande.}

Le vecteur d'état $\boldsymbol{x}$ est ici produit selon un principe de ``codage par population'' \shortcite{Georgopoulos1986,Averbeck2006}, autrement dit via un échantillonnage de $P_\Theta$. 
...
\subsection{Cas de la rétroaction positive}
...
Par rapport au cadre classique de la commande, la difficulté en modélisation repose sur la multiplicité des tâches (réalisations motrices) accomplies par le couplage cerveau-environnement. 
...
Une approche courante en modélisation est de stipuler l'existence d'une chaîne de traitement hiérarchique, 
...
Différents algorithmes permettent, comme nous l'avons vu dans la section précédente, de 
construire de telles chaînes de traitement de manière automatique à partir d'une simple 
base d'exemples, via des mécanismes de'''auto-encodage''.
...
Les réseaux de neurones récurrents sont donc capables de maintenir une activité persistante en l'absence de stimulation extérieure. 
...
L'identification des dynamiques de relaxation à des opérateurs
d'appariement signifie essentiellement~:
\begin{itemize}
	\item que la dynamique de relaxation doit 
	...
	\item qu'il existe des mécanismes de plasticité permettant de faire ``bouger les frontières''
	entre les différentes régions, voire d'en créer de nouvelles et d'en faire disparaître. Le modèle de plasticité synaptique le mieux établi est la \textit{règle de Hebb} \shortcite{Heb49}, confirmée à de nombreuses reprises par les observations \shortcite{Bli73,BIPOO98}. 
...
Le but est d'identifier la mise en œuvre d'un mécanisme d'appariement dans des réseaux de neurones récurrents soumis à la règle de Hebb.
...
\paragraph{Réseaux de neurones récurrents aléatoires} Au sein de ce problème générique, nous étudions plus particulièrement le cas des réseaux de neurones définis par des graphes aléatoires.
...
\section{Expressivité des grands réseaux aléatoires}
...
\paragraph{Assemblage dynamique}	Une troisième approche, celle de la ``mémoire extérieure'' \cite{ORe01}
...
(voir eqs. (\ref{eq:optim_input}) (\ref{eq:SD})).
