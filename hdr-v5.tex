...
\subsection{Problème des disparités d'échelle}
...
L'amélioration du processus repose alors sur le fait de trouver de nouveaux paramètres $\Theta'$ en estimant la vraisemblance des paramètres étant donné l'état latent, soit en marginalisant~:
\begin{align}
	\mathbb{P}'(\Theta) = \int \mathbb{P}(\Theta|\boldsymbol{x},\boldsymbol{I},\boldsymbol{u}) \hat{f}(\boldsymbol{x}) d\boldsymbol{x}
\end{align}
soit en maximisant~:
\begin{align}
	\max_{\Theta'} \int  \hat{f}(\boldsymbol{x}) d\boldsymbol{x} \label{eq:Friston}
\end{align}
...
$$\max_{\Theta',\boldsymbol{u}'}\int \hat{f}(\boldsymbol{x}) d\boldsymbol{x} %= \min_{\Theta',\boldsymbol{u}'} \int -  \log\hat{f}(\boldsymbol{x}) d\boldsymbol{x} 
$$

%{\color{Cyan} Dans le cas des modèles du cervelet $I$ est la conséquence proprioceptive de la commande.}
où la distribution a posteriori $\hat{f}(\boldsymbol{x})$ est produite de manière générative selon un principe de ``codage par population'' \shortcite{Georgopoulos1986,Averbeck2006}, autrement dit via un échantillonnage de $P_\Theta$. 
...
\subsection{Observations et modèles}
...
\chapter{Considérations supplémentaires}

{\color{Gray} Les travaux présentés dans ce mémoire sont centrés sur
...
Le travail de compilation et de relecture/réexamen de résultats parfois anciens liés à la rédaction de ce rapport a été l'occasion de développer quelques rapprochements qui ne sont pas présentes en tant que telles dans les publications. 
...
Pour les développer, nous avons utilisé un certain nombre de parti-pris méthodologiques~:
 \begin{itemize}
	...
	\item Nous adoptons l'hypothèse d'architectures de contrôle basée sur l'opérateur d'appariement. En particulier nous cherchons à généraliser le principe d'appariement (``pattern matching''), à l’œuvre en apprentisage automatique, au cas des systèmes dynamiques non-linéaires bouclés.
...
\subsubsection{(i) Séparation des signaux} L'équation de récurrence:
...
\subsubsection{Appariement conditionnel}
...
La thèse principale développée ici est que :
\begin{itemize}
	\item[]
...
\begin{itemize}
	\item De nombreuses études permettent de lier 
	l'activité ``microscopique'' de ces systèmes à des grandeurs ``macroscopiques'' basées sur les comportements moyens et les corrélations d'activité \shortcite{Wilson1972,Ama72,Amit1985,Som88,Ces94B,Ami97}, etc.
