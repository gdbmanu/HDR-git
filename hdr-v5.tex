...
se développent depuis une quarantaine d'années, visent à mieux comprendre l'organisation des circuits neuronaux selon la double perspective du traitement et
de la transformation des signaux sensoriels, d'une part, et du contrôle et de la coordination des actions, d'autre part. 
Il s'agit, au niveau microscopique, de mieux comprendre le sens des signaux émis par les cellules nerveuses, mais également, au niveau ``macroscopique'', de mieux comprendre la manière dont le système nerveux des êtres vivants organise 
leur environnement sensoriel et moteur. 
Le développement rapide des neurosciences invite à constamment réviser les principes fonctionnels qui y sont à l’œuvre.
...
Ces travaux reposent, comme nous le verrons, sur un certain nombre de parti-pris méthodologiques~:
\begin{itemize}
	\item Utiliser le cerveau comme source d'inspiration. La plasticité de Hebb, en particulier, est considérée comme le modèle de référence pour les algorithmes d'apprentissage.
	\item Généraliser le principe d'appariement (``pattern matching''), à l’œuvre en apprentisage automatique, au cas des systèmes dynamiques non-linéaires bouclés.
	\item Implémenter un calcul \textit{distribué} par les réseaux de neurones récurrents, de sorte que l'ensemble du calcul soit réparti à parts égales sur les différents nœuds du réseau. Le ``temps de calcul'' correspond au temps de relaxation vers un \textit{attracteur}. % Ce calcul est réalisé par l'ensemble des neurones du réseau. 
\end{itemize}
...
Le travail de relecture et de compilation nécessaire à la rédaction de ce rapport a également été l'occasion de mettre en lumière certaines difficultés fondamentales liées à la modélisation de l'apprentissage de compétences motrices.
Il existe en effet peu d'exemples dans la littérature où les principes de l'apprentissage automatique 
aboutissent à l'émergence d'une loi de contrôle dans un espace d'actions continu.
Ce rapport propose donc quelques pistes de recherche pour y remédier, au travers de principes de contrôle moteur fondés sur la rétroaction \textit{positive}.
...
\subsubsection{Assemblées neuronales} Le principe de plasticité par conjonction d'activité est articulé, dans le livre de Hebb, avec le concept d'\textit{assemblée neuronale} \cite{Heb49}. 
