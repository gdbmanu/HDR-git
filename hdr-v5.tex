...
l'idée de \textit{circuit reconfigurable}, s'appuyant sur une \textit{activité endogène} produite par un réseau de 
...
La fonction de réponse $h$ (voir équation (ref{eq:SIG}), page \pageref{eq:SIG}) est en général une projection sur un espace $\mathcal{U}$ de dimension plus petite que l'espace d'entrée $\mathcal{I}$.
...
La plupart des algorithmes d'apprentissages s'affranchissent de la contrainte de localité définie par Hebb et implémentent des mécanismes de mise à jour reposant sur la propagation d'erreur (et non sur la conjonction d'activité).  
...
\paragraph{}
La correspondance formelle entre l'apprentissage (au sens informatique) et la plasticité (voir équation (ref{eq:SD-plast-x}), page \pageref{eq:SD-plast-x}) consiste à identifier~:
\begin{itemize}
...
Dans le cadre du contrôle tel présenté précédemment, l'apprentissage consiste donc à modifier le contrôleur décrit par la fonction  $\phi_\text{in}$ (voir l'équation (\ref{eq:SD-closed-in}), page \pageref{eq:SD-closed-in}, et l'équation (\ref{eq:feed-noise-3}), page \pageref{eq:feed-noise-3}) dans un sens qui améliore un critère de performance prédéfini. La mise à jour d'un modèle ou d'une fonction de réponse consiste en général à modifier un jeu de paramètres $\Theta$ qui décrit le modèle.
...
Les règles de plasticité Hebbiennes définissent une dynamique sur les liens synaptiques (voir les équations (\ref{eq:SD-plast-x}) et (\ref{eq:SD-plast-J}),
page~\pageref{eq:SD-plast-x}, et l'équation~(\ref{eq:plast-hebb}), page~\pageref{eq:plast-hebb}), selon les valeurs des activités pré et post-synaptiques. Dans la forme simple étudiée ici, a plasticité  
...
\begin{itemize}
	\item Les modèles de la perception fondés sur un appariement négatif \shortcite{Rao1999,Friston2009} font disparaître de la scène sensorielle les
	...
	\item Les modèles de la perception fondés sur un appariement positif ``complètent'' le signal d'entrée avec le signal prédit:
	\begin{itemize}
		\item Si l'écart est faible, le système tend à ignorer ce changement
		et persévère dans sa prédiction.
		\item Si l'écart est important et/ou réitéré, le système met
...
\begin{enumerate} 
	...
	\item Nous cherchons à identifier des mécanismes d'appariement sensori-moteurs compatibles avec le paradigme de l'apprentissage Hebbien et de la formation d'assemblées neuronales. Plus généralement, dans le cadre des systèmes dynamiques couplés (voir les équations (\ref{eq:feed-noise-1})-(\ref{eq:feed-noise-4}), page \pageref{eq:feed-noise-1}), nous regardons le principe de l'appariement conditionnel vu dans le chapitre \ref{chap:app}.
