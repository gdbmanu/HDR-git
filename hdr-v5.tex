...
\subsection{Réseaux récurrents aléatoires}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item {\color{Cyan}Nous montrons également, dans le cas d'environnements non-stationnaires }
	\end{itemize} 
...
\subsubsection{Espace visuel et espace praxique (pré-moteur)}
Si nous regardons maintenant l'activité sensori-motrice telle qu'elle se déploie à l'échelle macroscopique, on constate que l'activité humaine est fortement guidée par la composante visuelle.
...
Cette description des transformations sensori-motrices repose implicitement sur l'hypothèse d'un espace physique extensif où prennent place des transformations linéaires.  
...
%	%moduler une réponse à partir d'un écart à une valeur de référence par exemple, 
	Ce problème du lien entre les différentes manifestations sensorielles d'un même objet s'appelle le problème de l'``assemblage'' (``binding problem''). Il a été identifié de longue date dans les sciences cognitives sans qu'une solution définitive ne  puisse lui être apportée \shortcite{VdM86}.{	\color{Gray}
...
Plusieurs pistes se présentent à nous~:
\begin{itemize}
	\item La commande balistique / par champ de vecteurs ne nécessite pas référence à un point d'opération [REFS]. La dynamique de relaxation et les constantes de temps propres à l'environnement contrôlé ``conduisent'' les actionneurs vers leur point d'équilibre. 
	\begin{itemize}
		\item L'avantage de ce type de commande est qu'elle définit l'espace péripersonnel comme un ensemble de ponts fixes que la main peut atteindre.
		\item seul compte le point d'opération final. Il y a donc une commande unique quel que soit le point de départ.
	\end{itemize} 
	\item l'alternance entre un ancrage pré-moteur et un ancrage  visuel permet de constituer un double mécanisme d'appariement prédictif permettant de construire un espace métrique péricorporel (en situant chaque point de l'espace visuel dans l'espace péricorporel, et inversement).
	\begin{itemize}
		\item organisé autour du triplet direction du regard (u1) / point d'opération (u2) / scène visuelle (I)
		\item doit s'appuyer sur une activité persistante
		\item déplacement moteur dans une scène visuelle fixe = changement du point d'ancrage moteur = changement d'objet; apprentissage des différents points (lieux d'objets)de la scène visuelle I.
		\item Les différents points de la scène visuelle sont appariés aux différents points de la scène prémotrice selon ce point d'ancrage visuel spécifique.
	    \item construction : mouvement de la main vers la fovea (u1,u2,I) $-->$ (u1,u2',I')
		\item déplacement du regard dans une scène motrice fixe = changement du point d'ancrage visuel. 
	\item les différents points de la scène prémotrice sont appariés aux différents points de la scène visuelle selon ce point d'ancrage prémoteur spécifique (??).
	\item construction : mouvement du regard (foveation) vers le point d'ancrage prémoteur.  		
    (u1,u2,I) $-->$ (u1',u2,I')
	\end{itemize}    
\end{itemize}
...
déjà mentionné dans la section \ref{sec:NField}. 
...
peut donc être observée à l'échelle de quelques millisecondes, mettant en jeu un faible nombre de spikes par neurone.
