...
\paragraph{Taux de décharge instantané}
...
\paragraph{Modèles de champ neuronal}
}
...
La fonction de réponse $h$ est en général une projection sur un espace $\mathcal{U}$ de dimension plus petite que l'espace d'entrée $\mathcal{X}$.
...
\begin{itemize}
	...
	$f(U=u|I=\boldsymbol{I};\Theta)$, déduite de la distribution génératrice et des observations à partir des formules de Bayes.
\end{itemize} 
...
Les règles de plasticité Hebbiennes définissent une dynamique sur les liens synaptiques (voir eqs. \ref{eq:SD-plast-x}, \ref{eq:SD-plast-J} et \ref{eq:plast-hebb}), selon les valeurs des activités pré et post-synaptiques. Dans la forme simple étudiée ici, a plasticité  
repose soit sur la conjonction des activités pré et post-synaptiques, i.e~:
$$\dot{J}_{ij} = F({J}_{ij}, s_i^\text{out} \times s_j^\text{in})$$
(on parle alors de règle d'ordre 0),
soit sur des différences d'activité, i.e. 
$$\dot{J}_{ij} = F({J}_{ij},(s_i^\text{out} - m_i^\text{out})  \times(s_j^\text{in}-m_j^\text{in}))$$
...
Dans le cadre de systèmes dynamiques ouverts (c'est à dire soumis à un signal d'entrée, voir eq. \ref{eq:recurrence}), nous mettons en évidence deux mécanismes de réduction distincts~:
\begin{itemize}
...
les entrées similaires au motif appris, en produisant une réponse spécifique, et les entrées non-similaires, en produisant une réponse générique.
Le réseau peut donc être vu comme un ``réservoir'' de réponses dynamiques morphologiquement (ou qualitativement) distinctes
construites par le mécanisme de plasticité en présence de signaux présentés  de manière répétée.  


\subsubsection{z. La plasticité induite et le principe de trace synaptique implémentent l'apprentissage par renforcement dans des espaces d'action continus.}
...
\end{align}
(voir aussi eq. \ref{eq:SD-plast}) avec $\boldsymbol{S}_i(t) = \{s_i(t)\}_{t \in [t_0,..,t[}$ l'activité post-synaptique, $F$ la fonction de mise à jour et $\boldsymbol{y}(t)$
...
$$ \dot{J}_{ij} = F_J(Z_{ij},\boldsymbol{y}(t))$$
