...
\begin{enumerate}
	...
	\end{itemize}
\item Le second pôle concerne l'étude des dynamiques de populations de neurones. Les neurones sont dans ce cadre des unités de traitement simplifiées (neurones à spikes, neurones stochastiques, neurones fréquentiels,...). Le but est de comprendre les interactions entre populations de neurones, et d'étudier les conditions d'émergence de certaines signatures d'activité (corrélations large échelle, couplages de phase, champs récepteurs, ...) observés en électrophysiologie chez le sujet sain et dans les pathologies nerveuses.

	Les principales avancées dans ce cadre sont~:
	\begin{itemize}
		\item L'activité spontanée du cerveau, qui est (i) désordonnée, (ii) de faible intensité, (iii) avec une réponse linéaire aux fluctuations d'entrée, (iv) et des corrélations à large échelle \cite{Palva2013},
		est une activité de ``haute conductance''  \shortcite{Van98,brunel00,Destexhe2003}, correspond à un équilibre dynamique entre les influences excitatrices et les influences inhibitrices. Les neurones travaillent dans un état ``critique'', c'est à dire très proches de leur seuil de décharge.
		\item Modélisation de l'activité persistante de population sur des ensembles de neurones 
