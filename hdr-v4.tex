...
A l'autre extrémité du spectre,  l'apprentissage automatique
...
\begin{itemize}
	...
	\item Les approches explicatives \shortcite{Bishop2006}
...
Ce mécanisme d'apprentissage en ligne a l'avantage de rester valide lorsque les donnéessont non-stationnaires Ainsi les régularités observéees durant les premières phases de l'apprentisage ne sont plus nécessairement présentes
...
Le principe de programmation par conjonction d'activité est articulé, dans le livre de Hebb \cite{Heb49}, avec le concept d'\textit{assemblée neuronale}. 
...
La notion d'assemblée est donc liée à la notion d'activité~: une assemblées est un patron d'activité qui réapparaît fréquemment (l'activité d'un nœud étant identifiée à un état ``haut'').
...
La règle de Hebb s'interprète dans le cadre des réseaux de neurones récurrents. 
Nous avons vu que les réseaux de neurones récurrents sont capables de développer une activité propre (activité ``endogène''), pouvant présenter des régularités liée aux régularités du graphe ou liée à une influence extérieure (activité induite). 

La règle de Hebb s'interprète comme le fait de capturer dans la structure du réseau les régularités statistiques (covariances) présentes dans cette activité spontanée. Une assemblées apparaissant fréquemment tend à s'inscrire dans le graphe de connexions, ce qui tend à favoriser sa réapparition dans le futur.
Une assemblée est donc une forme d'activité caractéristique, sélectionnée (stabilisée) par la règle de plasticité. 

La règle de Hebb s'interprète comme la règle de ``\textit{plus du même}''. 
 
D'un point de vue computationnel, cette sélection/stabilisation s'apparente au mécanisme d'\textit{association} (ou couplage) entre les constituants d'une scène. On parle de mécanisme \textit{auto-associatif}, chaque constituant étant, selon le contexte, un opérande ou le résultat de l'opération.

En pratique, le mécanisme facilitateur de Hebb est couplé avec un mécanisme stabilisateur qui évite la divergence du processus (via un principe de sélection compétitive).  
...
{\color{Cyan}
	Idée de base~: étudier le rôle de l'activité récurrente dans le cadre de l'apprentissage. En particulier: reconnaissance de patrons spatio-temporels.   
	...
	On peut étendre à l'idée de vocabulaire comportemental/appariement exclusif/''winner takes all''/argmax.
...
\subsection{Principe d'appariement exclusif}

La plupart de ces algorithmes fonctionnent selon un principe d'\textit{appariement} (``pattern matching''), c'est à dire sur la construction, à partir des données, d'un ensemble de vecteurs ``caractéristiques''. Ces vecteurs caractéristiques, vus comme un un dictionnaire de formes, constituent une base de description synthétique des données d'entrée. La projection des données d'entrée sur cet espace de description facilite ensuite le traitement et l'interprétation des données.  L’apprentissage revient donc à la constitution d'un vocabulaire de base qui sert de clé d'interprétation des données d'entrée. 

{\color{Cyan} 
	Exemples :
	\begin{itemize}
		\item Appariement exclusif~: les barycentres des partitions génératrices permettant d'interpréter une donnée d'entrée comme membre d'une classe donnée. Principe de l'argmax ou du ``winner takes all''. Non-linéaire. \textit{Enumeration.}
		\item Appariement non exclusif : les données sont interprétées comme une combinaison de facteurs.  projection des données sur une base de plus petite dimension. Matching pursuit. Dictionnaire. Combinaison linéaire. ACP, SVD, deep learning, ... \textit {Décomposition}.
	\end{itemize}
}
