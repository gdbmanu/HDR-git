...
Ce qui différencie principalement le calcul traditionnel du calcul distribué est la taille et la dimension des opérandes. Le calcul distribué s'adapte bien à la réalisation d'opérations matricielles dans les espaces de grande dimension. 

Le mécanisme fondamental sur lequel s'appuient la plupart des algorithmes d'apprentissage automatique est celui le l'\textit{appariement} entre un \textit{dictionnaires de formes} et un vecteur (ou un ensemble de vecteurs) d'entrée. 
...
Le choix de ces formes caractéristiques est bien sûr dépendant des capacités d'expression du programme sous-jacent, principalement~:
\begin{itemize}
	\item nombre de formes (taille du dictionnaire)
	\item types de dépendances pouvant être capturées (dépendances entre classes, hiérarchies, dépendances temporelles, etc.)
\end{itemize}
...
La construction du dictionnaire de formes s'appuie sur les contraintes exprimées par les données d'apprentissage. 

{\color{Cyan}

\begin{itemize}
\item Apprentissage supervisé et conditionnement simple

\item Apprentissage par renforcement et conditionnement opérant
...
\paragraph{Apprentissage en ligne}
...
\subsubsection{Apprentissage et mémoire}
...
Il est facile d'établir une correspondance formelle entre l'apprentissage (au sens informatique) et la plasticité, en identifiant les paramètres de la fonction de réponse aux poids synaptiques, et la plasticité à un mécanisme de mise à jour des paramètres guidé par les données.
...
La plupart des algorithmes d'apprentissages s'affranchissent le la contrainte de localité définie par Hebb et implémentent des mécanismes de mise à jour basés sur la propagation d'erreur (et non sur la conjonction d'activité).  
