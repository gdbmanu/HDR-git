...
L'ensemble $\mathcal{C}$,  continu ou discret, est l'ensemble des consignes (ou tâches) possibles.
...
Soit $\Theta$ un ensemble de paramètres pour décrire la fonction $\phi_\text{in}$. En tenant compte de la dynamique, le problème peut s'exprimer comme~:
\begin{align}
\max_{\Theta}  \int_{\boldsymbol{x}^* \in \mathcal{C}} \lim_{t\rightarrow\infty} ||\boldsymbol{x}^* - \boldsymbol{x}_\text{out}(t)||^2 d\boldsymbol{x}^*
\end{align}

\paragraph{Erreur motrice et erreur de prédiction}
...
\begin{enumerate}
	...
	\item Une autre approche consiste à comparer le signal observé au signal attendu  $h_\text{in}(\boldsymbol{x}^*)$. Ce signal attendu n'est pas directement observable. Il peut être \textit{reconstruit} via un \textit{modèle direct}. 
	\begin{itemize}
		...
		\item La commande motrice doit être choisie de telles sorte que l'erreur de prédiction (et non plus l'erreur motrice) soit minimisée.
	\end{itemize}
\end{enumerate}
...
\paragraph{Espace de la tâche}
	...
	Dans le cadre de problèmes de robotique, on distingue généralement~: 
	\begin{itemize}
