...
On utilise dans la suite le terme ``\textit{expressivité}'' pour désigner le nombre et la nature des modes produits par un réseau de neurones,
...
Il est possible, en tenant compte des temps de transport du signal sur les axones, 
de définir le patron d'activité au temps $t$ comme un flot~:
\begin{align}\label{eq:dif}
 \boldsymbol{s}(t) = f_T(\{\boldsymbol{s}(t')\}_{t' < t},\boldsymbol{I}(t))
\end{align}
qui définit un \textit{système dynamique} dont l'état est constitué de l'ensemble des PA
transitant sur les axones. 
La trajectoire du système $\{\boldsymbol{s}(t)\}_{t \in [t_0,t_f], \boldsymbol{s}(t_0)= \boldsymbol{s}_0}$ 
sur la plage temporelle $[t_0,t_f]$ est alors définie par l'intégration sur le flot de la condition initiale $\boldsymbol{s}_0$.
En pratique, l'équation (\ref{eq:dif}) est  une équation intégro-différentielle qui se résout
