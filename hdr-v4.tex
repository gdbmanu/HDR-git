...
L'amélioration du processus de filtrage repose alors sur le fait de trouver de nouveaux paramètres $\Theta'$ en estimant la vraisemblance des paramètres étant donnée l'état latent, soit~:
...
\item Dans le cas du contrôle avec consigne (voir section \ref{sec:consigne}), l'amélioration consiste à faire en sorte que le contrôleur obéisse mieux à la consigne, autrement dit minimise l'erreur motrice $\boldsymbol{x}^* - \boldsymbol{x}_\text{out}$. Soit $\Theta$ un jeu de paramètres définisant la fonction de transfert du contrôleur. Ce critère peut s'exprimer sous la forme d'un problème d'optimisation~:
\begin{itemize}
  \item avec mesure directe~:
  ...
  \item sans mesure directe, mais avec un modèle d'environnement:
   ...
   Il ne s'agit pas d'améliorer le modèle mais plutôt, en présence d'un contrôleur imparfait (ne conduisant pas à l'état désiré), de biaiser le modèle pour corriger la commande motrice dans un sens qui améliore le contrôle. {\color{Orange}[DEAN,ITO]}
  \item sans mesure directe, sans modèle d'environnement, à partir de l'observation $\boldsymbol{I}$ seule~:
  $$\min_{\Theta'}   ||h_\text{in}( \boldsymbol{x}^*) - \boldsymbol{I}||^2 $$
\end{itemize}
