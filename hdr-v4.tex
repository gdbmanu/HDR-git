...
\begin{itemize}
	\item  $\boldsymbol{\xi}_\text{in}(t)$ et $\boldsymbol{\xi}_\text{out}(t)$ sont les bruits d'état.
	\item  $\boldsymbol{\chi}_\text{in}(t)$ et $\boldsymbol{\chi}_\text{out}(t)$ sont les bruits de mesure.
\end{itemize}  
...
Le problème de filtrage consiste principalement à séparer le signal du bruit, à l'aide de la connaissance (ou \textit{a priori}) représenté par le modèle d'état.
...
{\color{Orange} Il s'agit d'une certaine façon d'améliorer le modèle de manière à prendre en compte les facteurs exogènes non modélisés.}
