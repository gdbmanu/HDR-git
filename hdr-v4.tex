...
Le filtrage utilise un \textit{modèle interne}, qui est un processus miroir de l'environnement, dans lequel la variable d'état \textit{interne} $\boldsymbol{x}_\text{in}$ évolue en ``imitant'' l'environnement. On note dans ce cas $\tilde{\boldsymbol{x}}_\text{out}=\boldsymbol{x}_\text{in}$ cette estimation, sur laquelle le contrôleur s'appuie pour choisir sa commande.
...
\begin{align}
& f(\tilde{\boldsymbol{x}}_0) = f_0(\tilde{\boldsymbol{x}}_0)\\
& g(\tilde{\boldsymbol{x}}_{t+1}|\tilde{\boldsymbol{x}}_{0:t},\boldsymbol{I}_{1:t},\boldsymbol{u}_{1:t}) \propto P(\tilde{\boldsymbol{x}}_{t+1}|\tilde{\boldsymbol{x}}_t,\boldsymbol{u}_t) f(\tilde{\boldsymbol{x}}_t|\tilde{\boldsymbol{x}}_{0:t-1},\boldsymbol{I}_{1:t},\boldsymbol{u}_{1:t-1}) \label{eq:accum-1} \\
& f(\tilde{\boldsymbol{x}}_{t+1}|\tilde{\boldsymbol{x}}_{0:t},\boldsymbol{I}_{1:t+1},\boldsymbol{u}_{1:t}) \propto
Q(\boldsymbol{I}_{t+1}|\tilde{\boldsymbol{x}}_{t+1}) g(\tilde{\boldsymbol{x}}_{t+1}|\tilde{\boldsymbol{x}}_{0:t},\boldsymbol{I}_{1:t},\boldsymbol{u}_{1:t})  \label{eq:accum-2}
\end{align}
où les termes de normalisation ont été omis, avec  $\tilde{\boldsymbol{x}}_{0:t}$ représentant une trajectoire particulière du processus stochastique et les densités de probabilité $f$ et $g$ portent sur l'espace d'états $\mathcal{X}$ du processus externe.
 
Le jeu d'équations (\ref{eq:accum-1}-\ref{eq:accum-2}) forme une équation d'\textit{observation} où la nouvelle estimation  $f(\tilde{\boldsymbol{x}}_{t+1}|\tilde{\boldsymbol{x}}_{0:t},\boldsymbol{I}_{1:t+1},\boldsymbol{u}_{1:t})$ se déduit de l'estimation précédente $f(\tilde{\boldsymbol{x}}_t|\tilde{\boldsymbol{x}}_{0:t-1},\boldsymbol{I}_{1:t},\boldsymbol{u}_{1:t-1})$ et de l'observation des nouveaux signaux $\boldsymbol{u}_t$ et $\boldsymbol{I}_{t+1}$. L'espace $\mathcal{X}$ est appelé l'espace des \textit{sources} de l'observation.
...
Ce modèle qui combine prédiction interne (récurrente) et correction via les entrées sensorielles et à la base des modèles de la perception. 
...
\begin{itemize}
	...
	\item Cas particulier où il n'y a pas de transition d'état. Le processus de mesure seul est filtré, aboutissant à un mécanisme d'\textit{accumulation d'évidence}. Principe de seuil de détection.
\end{itemize}

\paragraph{Exemples}~:
\begin{itemize}
	\item le filtre de Kalman. Le filtre de Kalman [KALMAN1960] utilise un modèle gaussien de bruit, défini par une moyenne et une matrice de variance-covariance {\color{Orange} évoluant au cours du temps} (??).
	L'environnement est modélisé comme un processus Gaussien soumis à .  
	\item les modèles de Markov cachés  
	\item les filtres à particules
\end{itemize} 
