...
Dans les modèles plus détaillés, le passage à l'état haut déclenche un mécanisme actif de réinitialisation qui ramène
...
neurones connectés sous la forme dun circuit récurrent (ou réentrant) [CITATION].
...
Si $n$ est le nombre de neurones, le nombre de paramètres du réseau est en $O(n^2)$.

Les activités des différents neurones sont interdépendantes du fait des couplages. 
L'activité de réseau $\boldsymbol{s}$ (ou activité de population ou encore \textit{patron d'activité}) est 
constituée de l'ensemble des signaux émis et transmis de neurone à neurone.
La fonction de réponse du réseau $f_T$ est une fonction paramétrique 
décrite par le tableau de paramètres $T$, telle que l'activité $\boldsymbol{s}^*$ est solution de~:
\begin{align}\label{eq:optim}
  \boldsymbol{s} = f_T(\boldsymbol{s})
\end{align}
avec :
$$\boldsymbol{s} = \{\boldsymbol{s}(t)\}_{t = t_0,..., t_f}$$
où $\boldsymbol{s}(t)$ est le vecteur d'état du réseau à l'instant $t$, 
$t_0$ est l'instant initial (début de l'observation) et $t_f$ l'instant final 
...
de neurones d'\textit{entrée} soumis à des données exterieures, qui sont donc les opérandes du calcul.
C'est le cas chez l'animal des cellules \textit{sensorielles}. 
On note $\boldsymbol{I} = \{\boldsymbol{I}(t)\}_{t = t_0,..., t_f}$ ce signal extérieur. 
...
En étendant la fonction de réponse aux entrées extérieures, cette activité $\boldsymbol{s}^*$ est la solution de~:
\begin{align}\label{eq:optim_input}
\boldsymbol{s} = f_T(\boldsymbol{s}, \boldsymbol{I})
\end{align}
On dit également 
...
Les équations (\ref{eq:optim_input}) et (\ref{eq:E/S}) permettent,
sans précision supplémentaire, de multiples solutions. 
L'existence ou non de ces solutions multiples repose sur
le détail des modèles de neurones utilisés, ainsi que du caractère récurrent du graphe de connexions.
L'existence de solutions multiples suggère de plus, pour un même circuit, de multiples
manières de traiter le même signal. 
%Le calcul distribué consiste à traiter en parallèle des données de grande taille. 

Il est souvent possible de regrouper ces solutions selon des caractéristiques communes
se traduisant par une valeur unique au niveau de la fonction de sortie $h$. 
On parlera des différents \textit{modes} du réseau de neurones.
On parlera dans la suite d'\textit{expressivité} pour qualifier le nombre et la nature des modes produits par un réseau de neurones,
en présence ou en l'absence de signal extérieur. Le nombre et la nature de ces modes
nous permettra de décrire son ``vocabulaire'', et d'estimer le type d'opérations
dont il peut être le support. 

\subsection{Apprentissage automatique}

Les réseaux de neurones sont principalement utilisés, en apprentissage automatique, pour
ségréger et catégoriser leurs données d'entrée. 

Les modèles de traitement séquentiel (``\textit{feed forward}''), % (encore dite approche ``distribuée''), 
tels qu'il se sont définis depuis le perceptron \shortcite{Rosen58}, reposent sur
une notion implicite de ressource limitée.
L'\textit{encodage} est le processus par lequel une classe a priori non bornée de signaux 
d'entrée sera traduit sous la forme d'un ensemble borné de patrons d'activité,
chaque patron étant le reflet, au sein du réseau, d'une classe de signaux. 
...
Le nombre de régions séparables augmente exponentiellement avec le nombre de neurones dans la couche  
couche cachée.
Le concepteur fixe à l'avance le nombre de neurones à mettre dans cette couche cachée,
...
propose une définition quantitative de l'expressivité (ou complexité) d'un classifieur~: la
dimension de Vapnik-Chervonenkis, qui correspond au nombre de pivots utilisé pour
séparer l	es données d'entrée. 
\end{itemize}

La programmation du réseau de neurones est un des piliers du champ de recherche 
dit de l'apprentissage automatique (ou ``\textit{Machine learning}'').
Le but d'un algorithme d'apprentissage est de modifier 
les paramètres du réseau dans un sens qui améliore le comportement du programme, selon les caractéristiques
d'un jeu de données d'entrée ainsi que des sorties attendues [REFERENCES : PMC, Vapnik, LeCun, ].

\subsection{Modélisation biologique}

Dans le cadre de la modélisation biologique, une des voies de recherche est l'étude de mécanismes de calcul alternatifs au traitement
séquentiel standard. Le but n'est pas ici d'améliorer la performance des algorithmes de calcul, mais plutôt de comprendre 
certains des principes à l'{\oe}uvre dans le cerveau. 
Une des voies d'étude que nous regarderons dans le cadre de ce mémoire est 
l'idée de \textit{circuit reconfigurable}, basé sur l'existence d'une \textit{activité endogène} produite par un réseau de 
...
Le formalisme des systèmes dynamiques est couramment utilisé pour décrire l'activité 
des réseaux de neurones biologiquement inspirés. Il est principalement utilisé pour décrire les comportements
de réseaux de neurones 

détaillant de manière précise les dépendances temporelles entre les variables d'état 
du réseau de neurones.

Ce formalisme s'appuie sur une trame temporelle $\mathcal{T}$, décrite par une variable $t \in \mathcal{T}$,
un espace d'état $\mathcal{X}$ de dimension finie ou infinie, décrit par la variable d'état $\boldsymbol{x}(t) \in \mathcal{X}$, 
indexée sur la trame temporelle, et un flot $\Phi(x,t)$ tel que $\forall \boldsymbol{x},t$:
$$ \frac{d\boldsymbol{x}}{dt} = \Phi(\boldsymbol{x},t)$$
L'état du système sur la plage temporelle $[t_0,t_f]$ est défini par l'intégration sur le flot de la condition initiale $\boldsymbol{x}_0$. 
Le signal $\{\boldsymbol{x}(t)\}_{t \in [t_0,t_f], \boldsymbol{x}(t_0)= \boldsymbol{x}_0}$ est la \textit{trajectoire} 
du système issue de $\boldsymbol{x}_0$.
