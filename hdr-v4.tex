...
La fonction de réponse du réseau $\boldsymbol{f}_T$ est une fonction paramétrique 
décrite par le tableau de paramètres $T$, telle que l'activité $\boldsymbol{s}^*$ est solution de~:
\begin{align}\label{eq:optim}
  \boldsymbol{s} = \boldsymbol{f}_T(\boldsymbol{s})
\end{align}
avec :
...
\boldsymbol{s} = \boldsymbol{f}_T(\boldsymbol{s}, \boldsymbol{I})
\end{align}
On dit également 
...
\begin{align}\label{eq:E/S}
 \boldsymbol{u} = h(f(\boldsymbol{s}^*,\boldsymbol{I}))
\end{align}
...
séquentiel réalisé par les ordinateurs traditionnels.

%Le calcul distribué consiste à traiter en parallèle des données de grande taille. 

Un des buts de la modélisation biologique est d'étudier des mécanismes de calcul alternatifs au traitement
séquentiel standard. Le but n'est pas a priori deproduire de meilleurs algorithmes de calcul, mais plutôt de comprendre 
certains des principes à l'oeuvre dans le cerveau. 

Une des voies d'étude que nous regarderons dans le cadre de ce mémoire est 
l'idée de \textit{circuit reconfigurable}. Les équations (\ref{eq:optim_input}) et (\ref{eq:E/S}) permettent,
...
L'existence de solutions multiples suggère, pour un même circuit, de multiples
manières de traiter le même signal. On parlera des différents \textit{modes} du réseau de neurones.

Un exemple minimaliste est un réseau de neurones récurrent à deux neurones mutuellement connectés. 
Un tel réseau pourrait présenter deux modes : un mode ``bas'' global dans lequel les deux neurones
sont dans l'état ``bas'', et un mode ``haut'' global dans lequel une activité ``haute'' locale est transmise
indéfiniment d'un neurone à l'autre. Chacun de ces deux patrons d'activité est une solution possible
...
dont il peut être le support. Nous regardons en particulier le cas des réseaux de neurones 
...
des réseaux de neurones biologiquement inspirés. Il apporte un niveau de détail supplémentaire, en 
