...
\begin{align}
\min_{\Theta}  \int_{\boldsymbol{x}^* \in \mathcal{C}} \lim_{t\rightarrow\infty} ||\boldsymbol{x}^*(t) - \boldsymbol{x}_\text{out}(t)||^2 d\boldsymbol{x}^*
\end{align}
...
\begin{align}
\min_{\Theta}   \int_{t \in \mathcal{T}} ||\boldsymbol{x}^*(t) - \boldsymbol{x}_\text{out}(t)||^2 dt
\end{align}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item La commande motrice doit être choisie de telles sorte que l'erreur de prédiction (et non plus l'erreur motrice) soit minimisée {\color{Orange}[DEAN,FRISTON]}.
