...
traite un signal d'entrée $\mathbf{s}_\text{in}$.
La réponse du neurone est un signal $s_\text{out} = f(\mathbb{s}_\text{in})$, dit signal de sortie.
Les signaux sont indexés sur l'axe temporel, où $s_\text{out}(t)$ représente par exemple l'état du neurone au 
temps $t$.

Dans le cadre de modèles de neurones biologiquement inspirée, le signal $s_\text{out}$ est construit de telle sorte
qu'on puisse distinguer un état dit ``actif'' d'un état ``inactif'', où l'état actif signifie que le neurone transmet
de l'information et l'état inactif qu'il ne transmet pas d'information. 
Dans le cas le plus simple, la sortie au temps $t$ peut prendre deux valeurs : 0 ou 1. L'état 0 est l'état inactif, l'état
1 est l'état actif. Autrement dit l'état 0 ne "porte" aucune information. 
Dans le cas de modèles de neurones à impulsions, l'état actif est l'émission d'un potentiel d'action au temps $t$, l'état
inactif correspond à l'absence de potentiel d'action au temps $t$. 
