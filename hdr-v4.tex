...
La mise à jour d'un modèle ou d'une fonction de réponse consiste en général à modifier un jeu de paramètres $\Theta$ qui décrit la fonction en question. 
\begin{itemize}
\item Dans le cas des modèles de filtrage, il s'agit par exemple des paramètres des distributions $P$ (processus externe) et $Q$ (mesure). 
\item {\color{Orange} Dans le cas général, le jeu de paramètres est le ``programme'' .}
\end{itemize}
...
Cette succession d'estimations et de maximisations donne son nom à l'algorithme EM \shortcite{Dempster1977} qui, étant donnés les observations, converge vers une distribution limite $\hat{f}^*$.

{\color{Orange} consiste principalement à identifier cet état latent et la commande $\boldsymbol{u}$ se déduit de la différence entre cet état estimé et la consigne.
