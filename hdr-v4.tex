...
Cette section présente quelques notions plus spécialisées utiles à la compréhension de mes travaux. 
Ce sont principalement des notions spécifiques à l'étude de la plasticité des réseaux de neurones récurrents. 

Je m'intéresse aux réseaux de neurones récurrents en raison principalement de leur plus grande plausibilité biologique. Le but est d'identifier certains principes computationnels à l’œuvre dans ces réseaux.

Le choix de la plausibilité biologique implique de s'intéresser à la règle de plasticité de Hebb, confirmée à de nombreuses reprises par les observations. 
La plasticité de Hebb inscrit dans la structure du graphe les conjonctions d'activité pré et post-synaptique. 

Ne prenant pas en compte de signal d'erreur, cette règle est classiquement interprétée dans le cadre 
...
\end{itemize}
Ces modèles sont porteurs de limitations (en terme de capacité de stockage ou de preuve de convergence) qui rendent difficiles l'utilisation de la règle de Hebb dans le cadre de problèmes d'apprentissage automatique. 

La thèse principale développée ici est que la règle de Hebb, souvent sous-estimée pour ses moindres performances pratiques, doit être réinterprétée de façon approfondie dans le cadre des réseaux de neurones récurrents.
Autrement dit pour bien comprendre la plasticité de Hebb, il faut avant tout comprendre la logique à l’œuvre dans des réseaux de neurones récurrents, cette logique étant largement différente de celle des réseaux à couches.
...
{\color{Cyan}
	...
	On considère le cas de réseaux de neurones récurrents capables de maintenir une activité persistante
	
	{\bf Le principe d'appariement sélectif (à l’œuvre dans de nombreux algorithmes d'apprentissage) est réinterprété, dans le cas des réseaux de neurones récurrents, comme une réduction sélective de la complexité de la dynamique. 
	\begin{itemize}
		\item Cette réduction sélective est réalisée par la plasticité de Hebb en présence de signaux extérieurs contenant ``suffisamment'' de régularités.
		\item Elle se manifeste par l'activation d'un circuit d'activation spécifique (``assemblée'' de neurones)
		\item Cette réduction de complexité 
		correspondant à la relaxation du système dynamique vers un attracteur de dimension faible.
		\item La convergence vers l'attracteur n'étant pas immédiate, le temps de relaxation correspond à un ``temps de calcul''. Ce calcul est réalisé par l'ensemble des neurones du réseau. On parle de réponse de population (ne dépendant pas d'un neurone seul).
	\end{itemize}
	}
...
\subsubsection{Appariement distribué et intégration}
...
Contrairement aux classifieurs traditionnels qui apparient \textit{nécessairement} le signal d'entrée avec une des formes stockées dans la structure, certains réseaux de neurones 
réalisent des appariements conditionnels tels que~:
\begin{itemize}
...
\end{itemize}
Parmi l'ensemble des réponses possibles, le réseau produit la réponse ``je ne sais pas'' lorsque le signal présenté n'est pas suffisamment proche d'une forme connue.
...
On note $n \leq N$ le nombre de degrés de liberté sur lesquels évolue  \textit{effectivement} la dynamique. La valeur de $n$ s'interprète comme le degré de réduction opéré par le signal sur l'activité du réseau. 
\begin{itemize}
...
\end{itemize}
On parlera par la suite de \textit{réponse de population} (ou réponse collective)  lorsque le réseau ``réagit'' à une entrée particulière en réduisant son nombre de degrés de liberté.

Ce principe de sélection par la dynamique d'un réseau récurent a été exprimé dans les travaux de Walter Freeman \shortcite{ska87} dans le cadre d'un modèle de la perception des odeurs.
On retrouve une idée analogue dans les travaux de Haken \shortcite{Haken1983}, qui a conceptualisé le principe de réponse de population à travers la notion de \textit{paramètre d'ordre}, emprunté à la thermodynamique.

{\color{Gray} Un paramètre d'ordre est, dans le cas de l'étude des transitions de phase, un opérateur permettant de mesurer un type d'organisation spécifique. Il est nul
...
(comme celle d'un cristal par exemple).}
...
L'activité récurrente produit un signal et crée de la nouveauté. 
