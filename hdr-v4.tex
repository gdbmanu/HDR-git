...
Biologiquement parlant, il s'agit de petites interfaces d'échange chimique (``boutons'' synaptiques) situés à l'extrémité de l'arborisation terminale des axones. A l'arrivée d'un potentiel d'action, les synapses libèrent des neurotransmetteurs qui agissent sur les canaux ioniques des dendrites de la cellule post-synaptique (``libèrent'' des ions), ce qui a pour effet de modifier le potentiel de membrane de la cellule post-synaptique.
...
Au niveau du réseau entier, la plasticité
modifie les caractéristiques de la fonction de réponse des neurones. 
...
\end{align}
avec $t_0$ l'instant initial d'observation, $S_i(t) = \{s_i(t)\}_{t \in [t_0,..,t[}$, $S_j(t) = \{s_j(t)\}_{t \in [t_0,..,t[}$,
et $F$ la fonction de mise à jour des poids synaptiques.
...
Cette équation illustre l'interdépendance entre le mécanisme d'activation $f$ (qui ``produit'' le patron d'activité $\boldsymbol{S}$ agissant sur les synapses) et le mécanisme de plasticité $F$ (qui ``produit'' le patron d'interaction $J$ qui agit sur la fonction d'activation). 
Le mécanisme de plasticité introduit donc une interdépendance complexe entre le graphe, l'activité et le signal d'entrée.

Il existe, du point de vue biologique, des mécanismes de plasticité synaptique à court terme [REF STD], à moyen terme [] et à long terme [REF LTP]. On parle de potentiation à court terme vs. potentiation à long terme.
...
Cette dynamique lente a donc un impact sur le comportement du réseau de neurones sur le long terme. Il s'agit essentiellement, selon l'idée initiale de Hebb, d'un mécanisme 
...
La plasticité induite est un mécanisme  de plasticité déclenché par un signal extérieur. Il peut s'agir, dans un cadre biologique, d'un neurotransmetteur, ou d'une entrée synaptique, spécifiques. Ce signal extérieur $r(t)$ est, dans le cas le plus simple, un scalaire agissant comme un {\color{Magenta} switch} qui ``ouvre'' ou ``ferme'' le mécanisme de plasticité.

{\color{Orange} L'application alternée des plasticités hebbienne et anti-hebbienne ouvre par ailleurs la voie
...
\subsubsection{Cadre formel de l'apprentissage}

{\color{Cyan} ICI ajouter des maths dans le meme esprit que chap 2.}
...
\subsection{Reconnaissance de formes et apprentissage}

Reconnaissance de formes. Feedback positif / feedback négatif. Mémoire associative. interpolation. 
...
Le mécanisme de sélection/intégration (Freeman)
