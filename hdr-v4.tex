...
Un réseau de neurones aléatoire est un graphe orienté constitué de $N$ nœuds défini par la matrice des 
couplages $J = \{J_{i,j}\}_{i \in 1..N,j \in 1..N}$ où chaque lien $J_{ij}$ est issu d'un tirage aléatoire indépendant.
...
observables à différentes échelles. {\color{Orange} Le mécanisme par lequel une population de neurones peut se mettre à osciller 
...
L'étude que nous avons présentée \shortcite{Dau99A,dauce01a} étudie le comportement d'un réseau dit ``équilibré'' (voir section \ref{sec:balanced}). 
...
Nous étudions le rôle contrasté de deux paramètres de contrôle (voir Figure 2). 
\begin{itemize}
...
\cleardoublepage
\subsection{Criticalité des réseaux aléatoires}
