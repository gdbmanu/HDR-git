...
Ce sont principalement des notions spécifiques à l'étude des réseaux de neurones récurrents. 
...
La notion d'assemblée est donc liée à la notion d'activité~: une assemblée est un patron d'activité qui réapparaît fréquemment.
...
On note $n \leq N$ le nombre de degrés de liberté sur lesquels évolue  \textit{effectivement} la dynamique. La valeur de $n$ nous indique le degré de réduction opéré par le signal. 
\begin{itemize}
...
si le milieu étudié est dans une phase désordonnée. Il est ``actif'' lorsque le milieu obéit à un organisation spécifique
(comme un cristal par exemple).}
