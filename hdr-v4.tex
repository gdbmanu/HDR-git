...
Ce chapitre est consacré à l'analyse des capacités d'expression des grands réseaux de neurones. 
...
Certains neurones du réseau peuvent être soumis à des données exterieures 
(non produites par l'activité des cellules neuronales).
C'est le cas chez l'animal des cellules \textit{sensorielles}. 
On parle alors de neurones d'\textit{entrée} du réseau. 
On note $\boldsymbol{I} = \{\boldsymbol{I}(t)\}_{t = t_0,..., t_f}$ ce signal extérieur. 
Si le neurone $i$ est un neurone d'entrée, on a $I_i \neq 0$,  et $I_i = 0$ dans le
cas contraire.
Le signal extérieur est généralement considéré comme indépendant de l'activité du réseau de neurones.

En étendant la fonction de réponse aux entrées extérieure, l'activité du réseau $\boldsymbol{s}^*$ est une solution de~:
$$ \boldsymbol{s} = \boldsymbol{f}_T(\boldsymbol{s}, \boldsymbol{I})$$

On parle dans ce cas d'une activité \textit{induite} par le signal d'entrée. On dit également 
que le réseau de neurones \textit{transforme} le signal d'entrée.

La \textit{réponse} du réseau de neurones au signal d'entrée est le patron d'activité induit.
Il est possible de définir une fonction de sortie qui \textit{décode} ce patron d'activité. 
La \textit{sortie} du réseau est alors 
$$\boldsymbol{u} = h(\boldsymbol{s})$$
avec $h$ fonction de décodage (ou ``\textit{read-out}'').

La fonction d'entrée/sortie (fonction de transfert) du réseau de neurones est alors~:
$$\boldsymbol{u} = h(f(\boldsymbol{s}^*,\boldsymbol{I}))$$
(il peut y avoir plusieurs sorties possibles si la solution n'est pas unique)

La sortie $\boldsymbol{u}$ peut être vue comme le résultat d'un calcul réalisé à partir des données d'entrée. 
Dans la mesure où ce calcul est le produit de 
l'activité conjointe (décentralisée) des neurones du réseau, on parle de \emph{calcul distribué}, par opposition au calcul
...
des réseaux de neurones. Ce formalisme s'appuie sur une trame temporelle $\mathcal{T}$ décrite par une variable $t \in \mathcal{T}$,
...
L'état du système sur une plage temporelle $[t_0,t_f]$ est entièrement défini par les conditions initiales $\boldsymbol{x}(t_0)$ 
et le flot. Le signal $\{\boldsymbol{x}(t)\}_{t \in [t_0,t_f], \boldsymbol{x}(t_0)= \boldsymbol{x}_0}$ est la trajectoire 
