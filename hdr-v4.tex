...
\begin{itemize}
	...
	\item $\boldsymbol{x}_\text{out}$ est un vecteur d'état  décrivant le contrôleur \--- ou encore ``milieu'' logiciel (entrées sensorielles comprises).
\end{itemize}
...
\subsubsection{Apprentissage en boucle fermée}
...
qui améliorent les interactions du logiciel avec le milieu physique, \textit{en tenant compte des boucles de rétroaction entre les commandes motrices et les signaux sensoriels} \cite{Wiener1965}. 	
%L'apprentissage en boucle fermée 
Il consiste à programmer le contrôleur de telle sorte que les actions produites dans le milieu se conforment à un certain nombre de critères (ou contraintes) définis \textit{a priori}.	
...
\begin{itemize}
	...
	\item la stabilité et la reproductibilité des réponses, la robustesse, etc.
	\item la capacité à prendre en compte de situations nouvelles
	\item etc. 
\end{itemize}

Ces problèmes ont été analysés de longue date dans le cadre de la théorie de l'automatique (contrôle linéaire) et de la commande optimale, en particulier sous l'angle de la stabilité de la réponse à la commande {\color{Orange}[REFS]}.
...
\paragraph{Tâche et espace de la tâche}
...
Le problème d'optimisation consiste à programmer une fonction de réponse $\phi_\text{in}$ qui, étant donnée une consigne $\boldsymbol{x}^* \in \mathcal{C}$, conduit le milieu physique dans l'état désiré. Soit $\Theta$ un ensemble de paramètres pour décrire la fonction $\phi_\text{in}$. En tenant compte de la dynamique, le problème peut s'exprimer comme~:
\begin{align}
\max_{\Theta}  \int_{\boldsymbol{x}^* \in \mathcal{C}} \lim_{t\rightarrow\infty} ||\boldsymbol{x}^* - \boldsymbol{x}_\text{out}(t)||^2 d\boldsymbol{x}^*
\end{align}
Dans le cas d'un dispositif mécanique, $\boldsymbol{x}_\text{out}$ décrit par exemple les coordonnées spatiales d'un effecteur et $\boldsymbol{x}^*$ la position finale désirée.

La différence $\boldsymbol{x}^* - \boldsymbol{x}_\text{out}(t)$ est appelée l'\textit{erreur motrice}.
\begin{itemize}
\item Dans le cas le plus simple (environnement observable), cette erreur est assimilée au signal d'entrée. 


Lorsque l'espace $\mathcal{C}$ est discret, chaque  
...
Etablir le lien conceptuel entre commande motrice et consigne.
}

Définir la tâche dans le cadre d'un problème d'optimisation : établir une commande qui conduit le système dasn un état 
