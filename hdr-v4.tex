...
\subsubsection{Rétroaction négative vs. contrôle balistique}
 ...
 On oppose au contrôle par feedback négatif le contrôle dit \textit{balistique} dans lequel l'erreur motrice n'est pas mesurée. Le contrôle repose sur la consigne $\boldsymbol{x}^*$ seule.
...
Le cas le plus courant en robotique et en modélisation biologiquement inspirée correspond à une situation où l'erreur motrice n'est pas directement observable. Deux cas se présentent~: 
\begin{enumerate}
...
L'apprentissage consiste de manière générale à modifier au cours du temps le contrôleur décrit par la fonction  $\phi_\text{in}$ (voir eqs. \ref{eq:SD-closed-in} et \ref{eq:feed-noise-3}) dans un sens qui améliore un critère de performance prédéfini.

La mise à jour d'un modèle ou d'une fonction de réponse consiste en général à modifier un jeu de paramètres $\Theta$ qui décrit la fonction en question (en d'autres termes ce jeu de paramètres est le ``programme'').  

{\color{Orange} Dans le cadre que nous avons regardé jusqu'à présent (modèles de la mesure, filtrage),  l'état extérieur $\boldsymbol{x}_\text{out}$ est l'état caché ``latent'' à estimer.
Le produit du processus de mesure est une distribution $f$ portant sur l'espace des variables latentes, reposant sur un modèle de la mesure paramétré par $\Theta$, noté en abrégé $f(\boldsymbol{x}|\Theta)$.
Le processus de mesure étant décrit par $\Theta$,  son amélioration  repose sur le choix de paramètres qui réduisent l'incertitude sur la valeur de $\boldsymbol{x}_\text{out}$, autrement dit 
$$-\min_\Theta \int f(\boldsymbol{x},\Theta) \log f(\boldsymbol{x},\Theta) d\boldsymbol{x}$$
où $f(\boldsymbol{x},\Theta)$ est la loi jointe. L'amélioration de la mesure consiste à réduire l'incertitude sur $\boldsymbol{x}$ qui s'exprime comme :


$f(\boldsymbol{x}|\Theta) f(\Theta) =
f(\Theta|\boldsymbol{x}) f(\boldsymbol{x})$



a fonction de réponse $f$, telle qu'on  est 

consiste principalement à identifier cet état latent et la commande $\boldsymbol{u}$ se déduit de la différence entre cet état estimé et la consigne.
Améliorer la fonction consiste 

L'apprentissage consiste alors principalement, étant donnés 
}
...
{\color{Orange} Le critère de qualité correspond en général à une mesure effectuée sur le dispositif physique en cours d'utilisation, dans le cadre d'une procédure de test ou de calibration.  }
