...
les paramètres du réseau dans un sens qui améliore la réponse, en fonction 
d'un jeu de données d'entrée ainsi que des sorties attendues [REFERENCES : PMC, Vapnik, LeCun, ].
Les réseaux de neurones sont principalement utilisés, dans ce cadre, pour
ségréger et catégoriser leurs données d'entrée. 

Les modèles de réseaux de neurones, % (encore dite approche ``distribuée''), 
tels qu'il se sont développés depuis le perceptron \shortcite{Rosen58}, reposent sur
un principe de traitement séquentiel (``\textit{feed forward}'').
%une notion implicite de ressource limitée.
L'\textit{encodage} est le processus par lequel un jeu a priori non borné de signaux 
d'entrée se traduira sous la forme d'un nombre fini de modes.
...
Le nombre de régions séparables (le nombre maximal de modes) augmente exponentiellement avec le nombre de neurones dans la couche  
couche cachée.
...
Il ne s'agit pas en premier lieu d'étudier la performance de ces algorithmes de calcul, mais plutôt 
d'identifier des points communs avec  
les mécanismes à l'{\oe}uvre dans le cerveau. 

Les réseaux de neurones récurrents sont définis par le fait que leur graphe contient des cycles.
Ces cycles induisent des dépendances réciproques entre les unités du réseau. 
...
Un systèmes dynamiques s'appuie sur une trame temporelle $\mathcal{T}$, décrite par une variable $t \in \mathcal{T}$,
...
Lorsque le système est dissipatifs, le flot définit un ensemble ... 
