...
Le but de ce document est de fournir un aperçu assez large des questions et des approches que j'ai 
eu l'occasion d'aborder au cours des vingts dernières années, depuis mes premiers pas dans la recherche. 
...
(étude des opératons du cerveau), les neurosciences fonctionnelles (étude des rôles des différentes structures anatomiques), 
...
pour rejoindre le groupe des écoles centrales. En plus de l'activité d'enseignement, ma recherche s'est déroulée  
dans le laboratoire ``Mouvement et Perception'' (unité CNRS dépendant de la faculté de sciences du sport), 
dasn l'équipe de Reonoud Bootsma, puis au sein
de l'Institut de Neurosceicnes des systèmes (unité INSERM de la faculté de médecine de Marseille),
dasn l'équipe de Viktor Jirsa. 
Cette activité de recherche s'est donc toujours déroulée dans un contexte impliquant des collaborations avec des
cliniciens, des psychologues expérimentaux, et des neuroscientifiques. Cet environnement a fortement influencé
...
Ce rapport évite volontairement les détails mathématiques, et insiste sur la mise en perspectives des travaux présentés
...
Ces travaux se situent à l'interface de la branche ``pattern matching'' (correspondance de forme) de l’IA  et des 
neurosciences. 
Le premier chapître, qui sert d'introduction aux thématiques développées dans ce rapport, présente ces deux approches,
...
Dans la seconde section, nous abordons la question de substrat, en tant que  support de la communication (le hardware). 
...
La troisième série de résultats est regroupée sous le thème de la ``subjectivité perceptive''. Les modèles présentés
...
La quatrième section aborde la question plus globale de la formation des comportements moteurs. 
...
La cinquième présente une série de résultats relatifs à des projets plus directement orientés vers la modélisation des mécanismes perceptifs et moteurs
...
\chapter{Systèmes dynamiques, reconnaissance de formes et substrat neuronal}\label{sec:pat}
...
\section{Contrôle en boucle fermée et feedback négatif} 
...
Il existe de nombreux modèles utilisant ces principes de contrôle pour modéliser
l'activité nerveuse. On citera parmi les plus connus \shortcite{Rob75,Wolpert1998}. 

L'approche par feedback positif présentée précédemment
n'est pas exclusive du schéma traditionnel du guidage de l'activité motrice par l'activité sensorielle.
...
La communication est dans ce cas basée sur 
la recherche de correspondance à une référence (et non la différence).
Le feedback positif ne reflète pas une activité de contrôle, mais une 
association (un assemblage sensori-moteur) transitoire.
La décision est basée dans un cas sur la contre-action, dans l'autre sur l'accumulation 
d'évidence (émergence).
...
Si nous reprenons l'analogie nerveuse, le corps (les articulations du corps) décrit 
...
On parle d'un espace de description quantitatif pour décrire ce référentiel extérieur
...
etc. doivent être apprises ``par coeur'' \cite{Pouget1997} et ne peuvent résulter de la combinaison d'activités unitaires,
...
cas de la carte colliculaire vue précédemment. Le problème se pose lorsqu'il faut
...
\section{L'approche mixte}

La séparation entre d'une part l'approche systèmes dynamiques, d'autre part la reconnaissance de formes 
...
Les premiers modèles mariant les deux approches remontent historiquement aux années 60-70.
L'idée de la sélection d'un canal de réponse par résonance avec les signaux de l'environnement de trouve dans \shortcite{Greene1962}.
Les premiers modèles sur la formation des patrons de réponse visuelle \shortcite{HUBELWIESEL62} remontent également à cette époque \shortcite{Grossberg1970}. 
L'idée d'un système dynamique de grande dimension  
...
Notre étude a donc ouvert la voie à l'étude multi-population systématique des réseaux de neurones à connectivité aléatoire \shortcite{Faugeras2009,Cabana2013} 
...
\chapter{Modèles de la perception à feedback positif} \label{sec:FPos}
...
Certains travaux réalisés êndant la thèse de Mathias Quoy (1991-1994) \shortcite{Quo94}
...
La validation du modèle adaptatif Gaussien sur des données P300 a été présenté à \shortcite{Dau14b}.
...
dans les cortex latéralisés gauches prémoteur inférieur et pariétal. 
...
l'homme et chez l'animal \cite{Gibbon1977}.
...
estimation interne du temps passé \cite{Gold01,Simen2011}.
...
Nous avons regardé en particulier le cas de la probabilité variable (``variable foreperiod''), c'est 
que les stimuli apparaissant à des intervalles variant d'un essai à l'autre.
...
réparties.
Conformément à l'hypothèse de vraisemblance, le passage du temps permet en effet d'éliminer les hypothèses
portant sur les intervalles courts, donnant plus de poids aux hypothèses les plus tardives. 

Le processus implémenté utilise un principe d'accumulation d'évidence concurrente. 
...
sur le principe d'un biais ``optimiste''. Une autre hypothése serait que 
...
La contrepartie logicielle de cette subjectivité est un substrat (un système dynamique) 
inspirés de l'étude du système nerveux, capables de développer 
une activité intrinsèque, capables de traiter une influence extérieure et capables de plasticité.
...
Le résultat de ce schéma computationnel est un fonctionnement par ``sauts'' d'un attracteur à un autre, 
...
versatile, constituée de ``tableaux comportementaux'' successifs détachés les uns des autres,
ou une activité séquentielle, dans laquelle le nouveau tableau est ``déduit'' de l'ancien
(où l'ancien tableau forme un contexte favorable à l'apparition du nouveau;
où l'ancien tableau
persiste dans le nouveau)?

L'essentiel des modèles à feedback positif que nous avons étudiés jusqu'à présent
répondent à la première catégorie. Le schéma conceptuel initialement proposé par Freeman \shortcite{Ska87}
...
à l'état de base. Le retour à l'état de base efface toute trace du patron d'activité précédemment formé.
...
Nous abordons dans la section \ref{sec:pat} la question du support de communication, {\color{red} relativement au fait
...
Des études portant cette fois sur le cortex moteur montrent également des corrélations entre l'activité mesurée sur les neurones du cortex moteur et le
mouvement effectif, en particulier leur coextensivité temporelle (l'activité s'arrête torsque le mouvement s'arrête). 
...
trajectoire métrique à partir de l'activité de différents neurones \cite{Georgopoulos1986,Wessberg2000}.
...
comparables à la durée d'un déplacement, semblent susciter des mouvements
convergeant toujours vers la même posture finale, quelle que soit la position
de départ \cite{Graziano2002}. Il s'agit dans ce cas d'une commmande motrice
...
Si on se place selon la perspective de l'invariance du monde environnant, il est intéressant de
...
des objets dans l'espace de pré-moteur, il est possible de predire leur position dans le champ visuel étant
...
un bon support à l'invariance ainsi qu'à la prédiction sensorielle (``predictive coding''). 
...
computationellement très coûteux. Il est nécessaire néanmoins que le ``lien'' praxo-visuel entre les différents objets de la scène ne soit pas perdu
...
dynamique et (3) la mémoire extérieure.
...
notre grand-mère). [JENNIFER ANISTON NEURON]
Un tel {\color{red} objet [activité???]} est donc la continuation à travers
...
Une troisième approche, celle de la ``mémoire extérieure'' [NOE-O'REGAN]
reprend la tradition 
de l'approche écologique impulsée par Gibson [REF]. L'existence de 
