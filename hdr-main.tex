...
Selon l'approche ``discriminative'' \cite{Vap95}, le calcul repose essentiellement sur des produits scalaire entre ces données et 
...
Selon l'approche statistique  \cite{Bishop2006}, le bruit (le ``hasard'') est une des composantes principales de l'algorithme. 
...
Les approches descriptives extraients des représentants caractéristiques des données (vecteurs supports). Les approches explicatives 
...
Si les principes de fonctionnement élémentaires du système nerveux central (neurones, synapses) commencent à être bien connus, 
le fonctionnement intégré du système dans son ensemble reste l'objet de nombreuses conjectures.
L'utilisation de modèles mathématiques et de simulations informatiques permet de proposer des hypothèses de fonctionnement et/ou 
...
Selon le typologie traditionnelles proposée par David Marr \cite{Marr82}, 
...
%Ces niveaux peuvent aussi être assimilés à des niveaux de description,
%du niveau psychophysique (ou macroscopique), au niveau architectural (mésoscopique) et au niveau 
%microscopique (neuronal).

%Du point de vue architectural, l'étude d'organismes relativement primitifs permet 
...
%On distingue ainsi chez la lamproie trois grandes catégories d'activités motrices, à savoir l'orientation 
%(déplacement les organes des sens vers les stimuli), la locomotion (déplacement du corps) 
%et l'emploi (activité alimentaire).

Le point de vue computationnel est souvent assimilé au niveau macroscopique (ou psychophysique). 
L'étude des temps de réponse, des courbes d'apprentissage et des
taux d'erreur permet d'explorer les caractéristiques limites d'un comportement et d'exprimer
des contraintes sur le système qui l'implémente physiquement.
L'élaboration d'un modèle descriptif à large échelle permet alors d'émettre des hypothèeses 
sur les raisons pour lesquelles le comportement présente les caractéristiques observées. 

De même, le point de vue algorithmique est souvent assimilé au mésoscopique (ou architectural). 
%les opérations réalisées par le cerveau
%ne peuvent se réduire à de simples traitements de données issues des organes sensoriels. 
Nous avons vu dans les parties précédentes des modèles constitués de quelques 
centaines de neurones développant une activité autonome, s'organisent sous la pression d'un signal externe et 
se transforment sous l'effet de la plasticité synaptique.
...
autour de l'étude des comportements d'orientation. Dans le cadre du projet MAPS déjà présenté, j'ai participé à une étude portant 
sur l'orientation visuelle à partir des données anatomiques et électrophysiologiques du tectum et des neurones prémoteurs du tronc cérébral,
en collaboration avec Alain Guillaume, travaillant à l'Institut des Sciences du Mouvement, et Anthony Mouraud, qui a effectué un séjour
...
Daniele Schön (INCM, Marseille), j'ai également travaillé sur des modèles génératifs du comportement d'orientation temporelle,
en colaboration avec Gaurav Malhotra qui a effectué un séjour postdoctoral en 2010-2011 sur cette question.
...
Notre travail de modélisation a principalement porté sur le problème de la transformation visuo-motrice (ou visuo-spatiale), c'est à dire expliquer comment une
activité focale, telle qu'elle se développe sur le colliculus supérieur, se traduit sous la forme d'un déplacement moteur oculaire initialement défini par la position
(le barycentre) de la bulle sur la carte. 
