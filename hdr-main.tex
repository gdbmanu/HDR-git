...
l'existence de délais différentiés. Ce sont ces oscillations qui donnent au substrat 
...
réseaux de neurones à état et à temps discret. Il s'étend facilement à des réseaux de neurones intègre-et-tire (non publié).
L'activité produite repose néanmoins essentiellement sur la fréquence de tir des neurones (et non sur l'ordre de tir
ou la co-activation). Les propriétés obtenues ne sont donc pas différentes qualitativement de celles qu'on 
obtient avec des modèles à fréquence de décharge.
Ce modèle a été utilisé en tant que couche perceptive dans une architecture de contrôle \cite{Dau04b}. 
D'autres modèles visant une plus grande fidélité aux processus biologiques 
étudient l'effet de délais variables et de patrons de connexion hétérogènes (voir par exemple \cite{Roxin2005}),
...
à première vue une certaine analogie avec le matériel électronique, 
...
supérieure à celle des neurones à fréquence de décharge [CITATIONS -- Izhikevich, etc..].
...
Mes contributions dans ce domaine portent sur l'extension et la généralisation des modèles
récurrents aléatoires au cas des réseaux de neurones à potentiels d'action ainsi qu'aux 
patrons de connectivité séparant clairement neurones excitateurs et neurones inhibiteurs.
Ce travail a été initié dans le cadre du programme de collaboration de l'ACI en neurosciences
...
Ainsi, les régimes d'oscillations lentes synchronisées
...
Le choix de résolutions temporelle et spatiale variées permet d'estimer les éléments
...
à basse résolution).
Ainsi, le mécanisme de réseau produisant ces régimes d'oscillations lentes n'apparait pas conditionné 
au type de cellules utilisées (à sortie discrète ou continue), mais comme reposant plutôt 
sur les fréquences moyennes, le taux d'amplification et les délais de transmission entre les différentes populations.

Cependant, la présence de séquences d'activation spatio-temporelles (dites ``groupes polychrones''), 
plus spécifiques aux modèles à spikes, telles que postulées par [IZHIKEVICH] en tant
qu'unités opérationnelles élémentaires, peuvent en partie retrouvées dans nos réseaux aléatoires.
L'étude récente d'un modèle utilisant des unités neuronales un peu plus détaillées \cite{Dau14a} 
...
signal d'entrée peut expliquer les effets de mémoire à court terme mis en évidence dans
les réseaux aléatoires à spikes \cite{Dau06,henry07}, sur le principe du ``réservoir'' vu précédemment.
