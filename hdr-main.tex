...
items et (b) difficile de fermer la liste. Selon le point de vue que l'on adopte, raisonner (établir de faits) 
...
décrire des mécanismes qui vont construire et autonomiser la réponse d'un agent vis à vis des sollicitations sensorielles de son
environnement.
...
Derrière le terme substrat, nous entendrons ici principalement un domaine physique bien délimité, naturel (corps) ou artificiel (machine), 
...
Tous les modèles de l'approche distribuée, tels qu'il se sont définis depuis le perceptron [CITATION], reposent sur
...
Une définition quantitative populaire de l'expressivité d'un tel substrat est la dimension de Vapnik-Chervonenkis [CITATION], qui représente donc le caractère 
...
L'illustration la plus caractéristique d'un tel processus d'encodage est celle des réseaux de Kohonen [CITATION] pour lesquels un nombre fixé de centres mobiles 
...
le modèle ART de Grossberg [CITATION] qui ajoute à son modèle la possibilité de recrutement de
...
L'exemple des réseaux de Hopfield [CITATION] offre ainsi un éclairage différent sur l'expressivité d'un substrat en mettant en 
...
se limitant à un nombre de modes (attracteurs) distincts en $O(N)$, c'est à dire de l'ordre du nombre de noeuds [CITATIONS].
...
``Champ neuronal dynamique'' [CITATION] décrit un substrat à support  
...
L'approche ``Multi-échelle'' [CITATION] consiste à considérer un substrat dont les variables d'état évoluent (ou peuvent être décrites) 
...
(tout réseau de neurone étant néanmoins en principe implémentable sous forme de hardware). Cette démarche est 
celle que nous poursuivons ici en passant en revue un certain nombre de substrats logiciels neuro-inspirés 
en vue d'étudier leurs capacités de traitement et d'expression.
...
Notre approche consiste donc à analyser les capacités d'un réseau de neurones en tant que support potentiel d'opérations cognitives. 
Dans ces différentes études, nous n'abordons pas la question de la plasticité, ne regardons pas l'encodage ou le rappel effectif des signaux. 
Notre approche se concentre plutôt sur la notion de régime dynamique et d'\emph{attracteurs}. L'idée principale est que notre substrat
...
Les premières études sur ce  type de modèle ont été faites par Sompolinsky [CITATION] sur un système dynamique continu. 
L'analyse d'une variante à temps
discret a été proposée par Cessac et Samuelidès [CITATIONS]. La démonstration de cette propriété (dite de ``chaos local'') a été faite 
dans le cas discret par Moynot et Samuelides [CITATIONS].
...
le moments d'ordre 2 et la covariance dans le temps [CITATION]. 
...
Il est intéressant de constater que le processus stochastique limite, telle que décrit par les équations de champ moyen, 
...
un  comportement générique de destabilisation, appelé route vers le chaos par ``quasi-périodicité'' [CITATION Bergé Pommeau; Quoy Cessac].
...
cas la transition se fait brutalement, avec un passage discret d'un processus de type point fixe à un processus stochastique [REF?].
Cette frontière entre ordre et chaos est par principe intéressante puisqu'elle indique le lieu
où la variabilité des dynamiques, et donc potentiellement l'expressivité du réseau est la plus importante. 

En collaboration avec Moynot, Samuelides et Pinaud [CITATIONS], j'ai travaillé sur la définition et 
...
à la frontière, et entre l'extinction et l'activité soutenue, comme les oscillations synchronisées [Brunel] et/ou le chaos [VV-Sompo]. 

L'étude que nous avons présentée dans [ESANN'99, NPL 2001] étudie le comportement d'un réseau équilibré de ce type. Ce papier 
...
Ce modèle renouvelle l'approche large-échelle classique [WILSON-COWAN] en prenant en compte l'hétérogénéité des faisceaux
...
L'étude numérique proposée dans le papier fixe le rapport excitation/ inhibition à 1/2,
ce qui correspond à un régime de faible activité [REFERENCES]. 
...
une des continuations principales étant le papier [CESSAC - FAUGERAS]
...
Une étude proposée dans le 3ème chapitre de l'ouvrage que nous avons co-dirigé avec Agnès Guillot [REF] illustre bien le propos.
...
mise à jour du réseau. Une succession de régimes quasi-stationnaires séparés par des transitions brusques est mis en évidence.
...
Une autre étude, présente dans mon manuscrit de thèse [REF], développe l'analyse spatio-temporelle des patrons d'activité de type cycle limite.
L'existence d'une chaîne d'activation spontanée (synfire chain [CITATION]) est mise en évidence par analyse de la matrice de covariance. 
Différents modes (patrons d'activation spatio-temporels) se développent spontanément lorsque le réseau est soumis à des entrées statiques différentes [SAB 2000].
...
(autrement dit à sélectionner un mode dynamique périodique interne compatible avec la période imposée) [SAB2000].
...
présentés dans la section \ref{sec:}. 

L'intérêt du comportement spontané des réseaux récurrents à connnectivité aléatoires a été largement confirmé depuis. 
Une étude plus systématique de la réponse linéaire dans les réseaux récurrents [CESSAC-SAMUELIDES EPJ-ST] met en évidence 
...
computing'' proposés par l'équipe de Wolfgang Maas [CITATION] reposent sur le même principe : un réseau de neurones aléatoire récurrent soumis à un signal
...
sur des transformatios morphologiques des patrons d'activités (changements des distributions spatiales et temporelles des activités). Bien que reposant sur
...
utilisées en apprentissage automatique [COMPRESSED SENSING, EXTREME LEARNING].
...
La vocation des modèles de champ neuronal est principalement la modélisation d'un substrat neuronal spatialisé vu à ``large échelle'' 
...
Un réseau de type champ neuronal implémente simultanément une ``mémoire de travail'' (capacité à conserver la trace d'une stimulation passée), et 
un mécanisme de traitement (ici résumé à quelques opérations simples : émergence, extinction, fusion, poursuite) [REF SCHONER]. 
La combinaison de ces deux compétences (mémoire et calcul)
au sein d'une architecture unique fait en principe du neural field un environnement computationnel complet, bien 
que les références manquent encore sur la question [REF?]. 

{\color{Violet} une seule référence sur la question : \bf Universal neural field computation
