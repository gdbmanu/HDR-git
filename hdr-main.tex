...
Comme dans le cas du réseau de Hopfield, elle semble
dépendre linéairement de la taille du réseau.

Des changements marginaux de la règle de plasticité, en limitant drastiquement le nombre
de liens éligibles,
...
selon le même formalisme que l'étude sur les effets de synchronisation sur les réseaux équilibrés.
...
de certains signaux. Contrairement au modèle précédent, la contrainte externe se manifeste sous
...
La règle Hebbienne utilisée ici est une règle d'ordre 1 (basée sur les différences d'activité), qui
...
vers une activité constituée de grandes oscillations périodiques (et non vers un point fixe).
...
Dans sa configuration initiale, la couche primaire reçoit passivement le signal spatio-temporel (séquence 
périodique de motifs). Les liens feed-forward aléatoires transmettent ce signal à la couche 
secondaire. L'activité résultante est le résultat de la concurrence entre cette projection aléatoire
du signal d'entrée et de l'activité intrinsèque chaotique (entretenue par les liens latéraux
...
Dans ce cas, l'existence de correspondances statistiques entre l'activité interne et l'activité 
...
l'activation des liens de feedback crée une boucle d'amplification positive entre la couche associative
et la couche sensorielle, qui transforme la corrélation statistique 
en lien de causalité. Cette projection induite de l'activité interne vers la couche sensorielle
...
Le feedback correspond à la meilleure prédiction actuelle, tandis que l'entrée sensorielle correspond
...
et persévérer dans sa prédiction. Si l'écart est important et/ou réitéré, le système met
...
à celui des réseaux de Hopfield. Le réseau peut continuer par exemple à ``croire'' à des éléments de la scène visuelle
(sensorielle) non perçus dans le signal actuel, à cause de phénomènes d'occlusion par exemple.

Cette capacité de persévération est donc un mécanisme de stabilisation de l'activité, qui 
résiste aux changements sensoriels. Selon le degré de résistance, le système plus ou moins ``flexible''.
Il existe en pratique un seuil au delà duquel BLA BLA


Le cas limite correspond au réseau qui ignore totalement son entrée sensorielle, 
persévérant indéfiniment sur un illusion perceptive produite par l'activité interne (``hallucination'').
Le feedback positif est donc à double tranchant puisqu'il porte en lui le risque 
de ``croire trop fort'', autrement dit de 
