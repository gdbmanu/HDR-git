...
le système nerveux, l'appareil sensori-moteur et son milieu.
...
\paragraph{Pattern matching ou transformations isométriques?}
...
La traduction inverse d'un patron d'activité vers un espace métrique, et la prise en compte
des erreurs sensorielles nécessite que la notion de distance ne soit pas perdue au cours des transformations.
...
\paragraph{Bruit créateur ou bruit destructeur?}

L'opposition entre bruit créateur, producteur de formes, et bruit destructeur, c'est à dire perturbateur
...
Chaque déplacement produit une nouvelle version des scènes proprioceptives et extéroceptives.
Dans le cas de la rétine, ce ne sont pas les mêmes neurones qui traitent l'ancienne position sensorielle et
la nouvelle position sensorielle.
Le lien d'identité entre les points d'intérêt de l'ancienne scène et ceux de la nouvelle ne peut se déduire des contenus sensoriels
seuls. Le changement de posture produit 
par la commande motrice modifie le \emph{point de vue} sur une scène extérocentrée permanente. 
C'est donc le rapport entre ces agencements corporels et la scène invariante qui modifie causalement
...
la commande motrice et l'agencement postural final, qui implique donc un lien causal
entre la commande motrice et le point de vue sur les éléments invariants de la scène extérocentrée. 
Selon cette hypothèse, et d'un point de vue algorithmique, il doit exister 
...
la scène sensorielle (qui est donc la clé d'interprétation de la scène sensorielle).
...
l'a priori d'invariance stipule que les changements posturaux (position de l'oeil dans l'orbite)
...
Une même scène est donc ``flashée'' selon différents points de vue. Les points de vue sont 
chacun positionnés dans un référentiel tête ou un référentiel tronc comme une série de 
photos reconstituant un panorama.
Une série de saccades peut être vue come une série de couples (posture, scène visuelle).
...
Ce problème a longtemps été formulé sous la forme du ``binding problem'' (problème de la liaison): 
Tout objet (ou région focalisée) de la scène sensorielle induit une activité distribuée 
sur différentes régions corticales, chaque activité traduisant une des caractéristiques
...
cette activation globale, les caractéristiques qui relèvent d'un objet et celles qui relèvent d'un autre?
...
La nature de l'activité servant de support n'est pas connue à l'heure actuelle.
On peut imaginer par exemple des activités ``template'' correspondant à des dispositions (agencements) caractéristiques 
d'éléments (constituants et agencement caractéristique des éléments d'un visage par exemple).
Il faut imaginer des patrons d'exploration visuelle caractéristiques, des ``plans de visite'',
constituant le socle de cette unification (ou plus généralement des plans d'utilisation)
