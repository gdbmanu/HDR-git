...
L'organisation à large échelle de l'activité du cerveau nécessite l'emploi de techniques d'imagerie BLA BLA.
...
L'étude de ces mécanismes de reconnaissance par ``réduction'' de la dynamique a été initiée dans le cadre
...
Certains travaux préliminaires \cite{Quo94} avaient déjà montré l'effet de réduction produit par 
la plasticité Hebbienne dans les réseaux aléatoire récurrents à 
dynamique chaotique intrinsèque.

Le papier qui présente ces idées de la manière la plus complète est celui de Neural Networks, datant de 1998.
Il reprend pour ce qui me concerne des travaux réalisés pendant ma première année de thèse \cite{Dau98A, DAU98B}. 
Le cadre de l'étude est l'étude de la réaction (spontanée ou acquise) d'un réseau de neurones récurrent aléatoire
à des patrons (stimuli) également aléatoires. Les réseaux étudiés étant normalisés, 
...
Le changement d'activité produit par la surimposition du patron statique n'a pas de signification
particulière. Il juste est possible de distinguer la nouvelle organisation de l'ancienne, à partir de 
différents observables (indice de complexité, période interne, répartition des niveaux d'activité).

Il ne s'agit pas de multistabilité au sens classique puisque le stimulus fait partie des ``paramètres'' du
réseau. Ainsi, chaque stimulus distinct définit système dynamique distinct convergeant vers un attracteur distinct. 
Le fait de modifier le stimulus au cours d'une même simulation revient à modifier le système dynamique ``en cours de route''. 

Une règle de plasticité synaptique est ajoutée, correspondant à un dynamique lente sur les couplages, qui modifie 
le patron d'interaction selon une règle inspirée de Hebb \cite{Heb49} (à savoir que le lien entre deux neurones se renforce s'il
...
Une fois la session d'``apprentissage'' terminée, la présentation du stimulus appris tend à induire
une activité qualitativement différente, plus simple que l'activité induite par les autres stimuli. 
La plasticité permet donc d'obtenir une réaction spécifique à une forme particulière (ici une
forme arbitraire générée par un tirage aléatoire). La plasticité hebbienne, en renforçant les liens 
entre les neurones les plus actifs, tend à produire des classes de réactions distinctes (réaction acquise) face
à certains motifs dont les caractéristiques spatiales sont ``reconnues''. Le reste des motifs
ne présentant pas ces caractéristiques spécifiques, induit une réaction ``par défaut''  
(motifs quantitativement identiques mais qualitativement différents).
Ce mécanisme de séparation entre le connu et l'inconnu à travers des caractéristiques dynamiques
de la réponse rejoint l'hypothèse formulée par Freeman \cite{Ska87} à propos de la perception des odeurs~: 
activité simplifiée pour des odeurs connues, activité de fond chaotique pour les odeurs non familières.

TODO : impact du papier
...
L'essentiel des résultats de la thèse et des publication associées \cite{Dau00,Dau02} porte sur une architecture 
neuronale à deux couches permettant d'implémenter un mécanisme de perception de signaux spatio-temporels,
sur la base d'un apprentissage hebbien qui crée un lien de feedback positif entre une couche sensorielle
et une couche associative récurrente. 
...
Ils ont été poursuivis dans le cadre de l'ACI ``Temps et Cerveau'' (20), qui a permis d'étendre
certains résultats à des architectures plus complexes, 
