...
régions connectées, l'autre fournissant le ``poids'' de cette connexion en terme de densité de fibres.
...
Le connectome offre bien sûr une ouverture incomparable sur l'architecture du cerveau, qui vient en complément
de l'analyse des patrons d'activité observés dans l'exercice d'une tâche.
%(les mêmes grandes autoroutes sont présentes)
L'analyse ``structurelle'' vient donc en complément de l'analyse fonctionnelle.
Plusieurs indicateurs ont par exemple été proposées pour distinguer les noeuds les plus centraux (les plus ``importants'')
des noeuds périphériques. Il en résulte la mise en évidence d'un ``noyau central'' \cite{Hagmann2008}  
qui présente des points communs avec le patron d'activité ``par défaut'' observé en imagerie fonctionnelle 
(l'activité observée lorsque le sujet n'est pas occupé à accomplir une tâche) \cite{Raichle2001}. 

Le projet ``The Virtual Brain'' (TVB) \cite{Leon2013}, porté par Viktor Jirsa, vise à combiner la connaissance anatomique et connectomique pour 
produire des simulations de l'activité à large échelle du cerveau. Le modèle proposé repose d'une part sur la diffusion isotropique de
...
construits à partir du connectome \cite{Golos2014}. Chaque noeud du réseau est identifié à une masse neuronale. 
Partant de l'analogie avec les réseaux de Hopfield \cite{Hop82}, le but
était d'identifier les principaux le nombre et la disposition anatomique des
...
régionaux mis en évidence par une analyse purement structurelle \cite{Hagmann2008} ainsi qu'avec les composants indépendants 
...
Cette multistabilité temporelle conduit fréquemment le réseau vers des régimes d'activité plus simples (noeuds uniformément actifs
ou inactifs) par dérive lente de l'activité moyenne. 
Une multistabilité temporelle à long terme est néanmoins possible sur les modèles à ``feedback négatif global'',
sur lesquels le niveau d'activité globale est controlé par le feedback.
