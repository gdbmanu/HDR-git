...
Un modèle de substrat est un ensemble de variables d'état et un modèle de transitions d'état censé traduire ces changement de la matière.
...
seul un nombre limité de faits peuvent être pris en compte, et un nombre limité de réponses produites.
...
d'énergie limités, soumis à du bruit, etc...) et estimer le nombre et le type d'opérations qu'il est capable de produire. 


Les réseaux de neurones naturel nous offrent un tel exemple de substrat universel, dans la mesure où, à 
...
Tous les modèles de l'approche distribuée, tels qu'il se sont définis depuis le perceptron, reposent sur
...
{\color{Violet} - substrat à nombre d’etats fixe : }


Néanmoins, l'expressivité d'un substrat ne se limite pas au nombre de symboles. Elle repose également sur la nature du processus 
par lequel des données ou des signaux sont \emph{encodés} dans le substrat.
...
{\color{Violet}
- substrat à nombre d’etats combinatoire :

- espace discret d’attracteurs : switching. Notion de “pôle” (ou “mode”?).
}

L'exemple des réseaux de Hopfield [CITATION] offre un éclairage différent sur l'expressivité d'un substrat en mettant en 
avant le caractère distribué (non-local) de l'encodage. Contrairement à l'idée classique d'un noeud comme unité de représentation 
(ou unité de séparation dans le cas des classifieurs), l'expressivité du substrat est vue sous l'angle du nombre
de patrons distribués (patterns) pouvant être obtenus en réponse à une stimulation. Si l'expressivité se trouve en principe augmentée
...
se limitant à un nombre de symboles (attracteurs) distincts de l'ordre de $N$ (nombre de noeuds).
La capacité s'exprime ici en nombre de \emph{modes} (configurations atteignables) et non en nombre de symboles.

{\color{Violet} line attractors : possibilité de tracking ou de switch . EDP.}
...
du substrat, et porte en lui une capacité de représentation (ou de reproduction) 
...
L'approche ``Multi-scale'' [CITATION] consiste à considérer un substrat dont les variables d'état évoluent (ou peuvent être décrites) 
...
en vue d'étudier leurs capacités de traitement et de représentation.
...
Les premières études sur ce  type de modèle ont été faites par Sompolinsky [CITATION] sur un système dynamique continu. l'analyse d'une variante à temps
...
Le comportement d'une classe de réseaux (l'ensemble des réseaux définis par la même loi de tirage initial) peut ête décrit à la limite thermodynamique par 
...
représentative de l'activité qu'on peut attendre dans un réseau de taille finie issu d'un tirage particulier.

Il est intéressant de constater que l'activité ``aléatoire'', telle que décrite par les équations de champ moyen, a une traduction à taille finie
sous la forme d'une activité de chaos déterministe. L'existence d'un 
...
cas la transition se fait brutalement, avec un passage brusque d'un processus de type point fixe à un processus stochastique [REF?].
Ramenée à des réseaux de talle finie, l'existence d'une telle frontière entre ordre et chaos est par principe intéressante puisqu'elle indique le lieu
où la variabilité des dynamiques, et donc potentiellement l'expressivité du réseau est la plus importante. 

En collaboration avec Moynot, Samuelides et Pinaud [CITATIONS], j'ai travaillé sur la définition et l'étude du comportement à taille finie d'un modèle de 
réseau de neurones aléatoire (RNA) à deux populations. Il s'agissait ici de modéliser et reproduire certains comportements observés dans les 
réseaux de neurones naturels composés d'une population deneurones excitateurs et d'une population de neurones inhibiteurs. 
Diférents régimes dynamiques avaient déjà été mis en évidences au sein de telles architectures (oscillations synchronisées, )
