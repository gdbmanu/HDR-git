...
Les modèles de la perception basés sur un feedback négatif \cite{Rao1999,Friston2009} font disparaître de la scène visuelle les
éléments prédits, de sorte que, selon cette théorie, ce qui est ``perçu'' est l'erreur sensorielle,
...
mais présente une sensibilité au bruit de mesure (fluctuations aléatoire), et nécessite la mise en place d'un reset 
...
L'architecture proposée dans \cite{Dau02} implémente de manière plus explicite le principe de traitement
sélectif de la scène sensorielle, en séparant l'espace des sensations entre ce qui est connu (produit 
un changement qualitatif sous forme de résonance 
perceptive) et ce qui est inconnu (pas de changement qualitatif).
La reconnaissance est produite par le franchissement d'un seuil (seuil de ``vigilance''),
...
Le papier de la conférence ICANN 2001 \cite{Dau01} présente une version préliminaire du modèle perceptif à deux couches
...
Une étude de 2005 \cite{dauce05} montre l'effet symétrique des règles Hebbienne et anti-Hebbienne (d'ordre 1): 
...
pour produire des comportements nouveaux.

Les premières études que j'ai publiées sur cette question se situent dans le cadre applicatif de contrôle moteur \cite{Dau04b},
et seront détaillée dans la partie suivante. Les principaux résultats concernant l'application de la plasticité 
sur des réseaux équilibrés sont détaillés dans le papier EPJ-ST de 2007 \cite{Dau07}.
Il s'agit d'un papier un peu fourre-tout, qui offre une revue sur la plasticité dans les réseaux récurrents ainsi
que quelques résultats originaux.
Entre autres, le papier présente une étude sur l'effet d'une nouvelle règle de plasticité Hebbienne, 
la règle TD (qui présente des analogies avec la règle des différences teporelles de Sutton et Barto \cite{SUTTON98}), qui est 
une règle du premier ordre stricte basée sur la corrélation des différences d'activité.
...
qualitativement différents (ici de grandes oascillations snchronisées) associées aux motifs appris. 
