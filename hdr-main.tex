...
produites.
La maîtrise de l'interfaces comprend une phase d'apprentissage de la part du sujet, 
pouvant induire des modifications dans l'activité électro-encéphalographique 
dans le sens d'un signal plus net, mais aussi dans la répartition et les temps de réponse
...
Un classifieur probabiliste fournit pour chaque flash une probabilité (a posteriori) 
de l'événement P300.
Ce classifieur ayant été calculé sur un ensemble de sujets, la fiablité de cette réponse unique
est assez faible. Il faut donc 

Ce type de modèle est très proche dans sa formulation de l'apprentissage supervisé en-ligne 
de classifieurs multi-classes (comme l'algorithme du perceptron, ou encore le gradient logistique multi-classe)
\cite{Bishop2006}.
Il a été proposé dans le cadre du P300 par \cite{Kindermans}
Le cadre probabilistes  permet 
...
d'estimation en-ligne de la probabilité a posteriori d'appartenance à une classe. 

La formulation Bayesienne du problème de détection du P300 permet de construire 
un modèle qui exprime pour chaque symbole la probabilité qu'il soit le symbole désiré.
Chaque illumination d'un nouveau groupe de lettres permet de mattre à jour à la fois la probabilité
attachée aux lettres flashées mais également celle attachée aux lettres non-flashées.
