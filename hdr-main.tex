...
l'état de base, soit une désynchronisation, lorsque les conditions de la synchronisation ne sont plus remplies.
...
Nous allons maintenant essayer de voir quels sont les difficultés posées par cette question, et quels sont
les principales pistes permettant de l'aborder, ce qui nous amène à formuler un projet de recherche visant 
à fonder une méthode de développement d'architectures non versatiles.

Nous abordons dans la section \ref{sec:pat} la question du support de communication, relativement au fait
que la nature du support détermine le type d'information qui peut transiter par le support, ainsi que le
type de manipulations réalisables. 
...
\section{Pattern matching et systèmes dynamiques}\label{sec:pat}

La plupart des étude sur les réseaux de neurones suggèrent un mode de fonctionnement
intermédiaire entre le pattern matching et les systèmes dynamiques. 
...
feedback moteur sur des espaces sensoriels complexes.
...
\subsection{Feedback positif ou feedback négatif?} 
...
Il existe de nombreux modèles de l'activité de contrôle utilisant ces principes de contrôle pour modéliser
l'activité nerveuse. On citera parmi les plus connus \shortcite{Rob75,Wolpert1998}  

L'approche par feedback positif développée précédemment
n'est pas exclusive du schéma traditionnel du guidage de l'activité motrice par l'activité sensorielle (schéma
à feedback négatif).  
Autrement dit, rien ne permet d'exclure que l'activité nerveuse, 
...
non-linéairement séparables par exemple) a pour contrepartie le fait difficile la définition
...
ne respecte pas nécessairement les distances de l'espace initial.
On parlera de manière générale de l'espace ``qualitatif'' pour décrire cet espace de redescription, ou encore d'espace
...
patron d'activité (un attracteur) et une commande motrice, comme dans le
cas de la carte colliculaire vue précédemment. Le problème se pose lorsqu'il faut
moduler une réponse à partir d'un écart à une valeur de référence par exemple, c'est à
dire plus généralement lorsqu'il s'agit d'estimer, au sein de l'espace de description
qualitatif, des quantités relatives à différents
points de l'espace métrique ou de l'espace sensoriel. 
...
\subsection{L'approche mixte}

La dichotomie entre d'une part l'approche systèmes dynamiques, d'autre part la reconnaissance de formes 
...
Cette dichotomie implique un mode de fonctionnement composite, basé d'une part sur des rapports entre intensités (monde physique), 
...
L'idée de la sélectin d'un canal de réponse par résonance avec les signaux de l'environnement de trouve dans \shortcite{Greene1962}.
...
L'idée d'un système dynamique de garnde dimension  
...
notera cependant que cette comande peut également etre vue comme une comande
absolue dans le référentiel rétinien).
Par ailleurs eten même temps, la fréquence d'activité sur la carte colliculaire est fortement corrélée avec
...
Des études portant cetet fois sur le cortex moteur montrent également des corrélations entre l'activité mesurée sur les neurones du cortex moteur et le
...
un bon support à l'invariance ainsi qu'à la prédictin sensorielle (``predictive coding''). 
...
Par exemple, un simple ``parren matching'' nécessiterait un apprentissage par coeur de la conséquence visuelle 
de toutes les combinaisons de dispositions d'objets et de tous les changements de référentiel tronc-tête, ce qui parait 
compuationellement très coûteux. Il est nécessaire néanmoins que le ``lien'' praxo-visuel entre les différents objets de la scène ne soit pas perdu
...
Un tel objest est donc la continuation à travers
...
[ASSEMBLAGE DE CIRCONSTANCE - STRUCTUE PLASTIQUE VERSATILE - LIGANDS ENTRE DIFFERENTS PATRONS]
...
\subsection{Le bruit comme support de la persistance?}
