...
répondent à la première catégorie. Le schéma conceptuel initialement proposé par Freeman \cite{Ska87}
...
est mis en oeuvre dans le modèle perceptif à feedback positif que nous avons présenté \cite{Dau02}.
On peut également assimiler ce mécanisme au passage d'un état ``bas'' (non spécifique) à un état ``haut''
(spécifique), puis au retour vers un état ``bas'', un peu comme dans le modèle de \cite{Brunel2000persistent}.
Les modèles de neurones impulsionnels présentés dans \cite{Hen08B,Dau14a} montrent, à une échelle temporelle plus fine, une capacité
...
La section \ref{} aborde la question de l'invariance de l'environnement externe. En
...
La section \ref{} aborde la question des architectures séquentielles, de l'alternance
entre la prescription motrice et prescription sensorielle.
Nous décrivons alors des opérations caractéristiques qui devraient
être réalisées par le support pour autoriser l'implémentation des mécanismes 
séquentiels envisagés.
Enfin, nous étudions le rôle du bruit dans un tel dispositif.
La dernière section présente enfin une série de projets plus concrets développés dans le cadre 
...
contrôle moteur.
La démarche des systèmes dynamiques explique bien les mécanismes de formation des mouvements et  
du contrôle, mais n'offre pas de cadre propice à la plasticité. 
...
L'idée de la sélectin d'un canal de réponse par résonance avec les signaux de l'environnement de trouve dans \cite{Greene1962}.
Les premiers modèles sur la formation des patrons de réponse visuelle \cite{HUBELWIESEL62} remontent également à cette époque \cite{Grossberg1970}. 
...
de l'approche systèmes dynamiques lorsqu'il s'agit de construire des espaces sensoriels complexes.
...
Peut-on contrôler efficacement un corps sans faire appel à la notion d'attracteur? Peut-on 
...
L'opération algorithmique réalisée par le pattern matching est graduelle : degré d'appartenance
(ou de correspondance) à un patron donné, sur un intervalle borné. L'opération répétée sur de multiples patrons produit de 
...
Le pattern matching est néanmoins transformation non-linéaire  qui rend difficile la définition
...
La démarche des systèmes dynamiques repose à la base sur l'étude des comportements mécaniques
...
L'approche par feedback positif développée précedemment
...
\subsection{Le mariage de deux mondes}
...
\subsection{Bruit créateur ou bruit destructeur?}\footnote{L'opposition entre bruit créateur, producteur de formes, et bruit destructeur, c'est à dire perturbateur
...
décision sur un tirage aléatoire \cite{Auer1995}.
...
d'un processus aléatoire \cite{VEe2005}. 
...
\subsection{Nature de la commande motrice}
...
L'étude du système saccadique montre une activité motrice fortement corrélée avec 
...
Certaines études montrent également des corrélations entre l'activité mesurée sur les neurones du cortex moteur et le
mouvement effectif, mais corrélations ne permettent pas (ou permettent difficilement) de reconstruire une 
trajectoire métrique précise \cite{Georgopoulos1986,Wessberg2000}.
...
Une structure algorithmique simple est celle de la projection inverse, c'est à 
...
Une troisième approche, celle de la ``mémoire extérieure'' [NOE-OREGAN]
