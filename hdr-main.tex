...
Les schémas computationnel proposé est celui du feedback positif, autrement
...
Nous allons maintenant essayer de voir quels sont les avantages et également les problèmes posés par cette approche, et quels sont
les principales pistes permettant d'y remédier, ce qui nous amène à formuler un projet de recherche visant 
à fonder une méthode de développement d'architectures cognitives sur la base de quelques principes simples.


\section{Questions ouvertes et problèmes}
...
Nous faisons ci-dessous une liste des résultats qui confirment ou contredisent cette approche 
à partir de l'analyse du système nerveux et plus généralement de la problématique posée par le
fait de contrôler un corps dans un environnement.
Le but est également de soulever certaines des nombreuses questions sur les principes computationnels 
à l'oeuvre au sein de l'activité nerveuse.

\subsection{Pattern matching ou transformations géométriques?}
...
Si nous reprenpons l'analogie nerveuse, le corps (les articulations du corps) décrit 
...
ou désactivant différentes populations de neurones, alors les opérations élémentaires de translation, rotation, co-variation, 
etc. doivent être apprises ``par coeur'' et ne peuvent résulter de la combinaison d'activités unitaires.

Un problème se pose lorsqu'il s'agit par exemple de contrôler les déplacements du corps à partir des patrons d'activités
distribués induits par les récepteurs sensoriels.
En fait, le passage par un domaine de description qualitatif
ne pose pas de problème lorsqu'il s'agit d'associer (de manière univoque) un
...
L'approche de la cognition par correspondance de forme apporte donc un support efficace à la plasticité,
...
Le problème est qu'il ne semble pas exister à l'heure actuelle de consensus sur la nature de cette commande!
On peut distinguer deux hypothèses : une hypothèse de commande absolue (la commande désigne un point de l'espace extérieur),
et une hypothèse de commande relative (la commande produit un déplacement relatif).


Le pattern matching ne pose pas de problème lorsqu'il
s'agit de 
...
Par exemple, il est possible d'établir des corrélations entre l'activité mesurée sur les neurones du cortex moteur et le
...
\paragraph{Bruit créateur ou bruit destructeur?}\footnote{L'opposition entre bruit créateur, producteur de formes, et bruit destructeur, c'est à dire perturbateur
...
\paragraph{Feedback positif ou feedback négatif?} 
 n'est pas exclusif du schéma traditionnel du guidage de l'activité motrice par l'activité sensorielle (schéma
...
\paragraph{Espace métrique ou espace logique?}
...
\paragraph{Commande de vélocité ou commande de point fixe?}
...
de commandes posturales et de commandes de vélocité.
...
chaque déplacement (on se limite ici aux mouvements ballistiques) 
produit donc une nouvelle scène proprioceptive et extéroceptive.
Le lien d'identité entre %les points d'intérêt de 
l'ancienne scène et  la nouvelle ne peut se déduire des contenus sensoriels
seuls.
Par exemple, dans le cas de la rétine, ce ne sont pas nécessairement les mêmes neurones 
...
atteint pour différentes configurations motrices. Ces configurations sont congruentes au point final.
Si on considère l'attention portée à un objet particuler de la scène visuelle, différentes configurations
sensori-motrices sont congruentes à cet objet via un déplacement : un ensemble de couples (sensation subjective,
proprioception) est relié de manière congruente à l'objet via un déplacement moteur via une action potentielle spécifique
(conduisant au même résultat - identité du point final).
(Un même objet est alors défini par un ensemble de paires (état, action))

Attention, dans le cas primitives motrices, il y a identité entre l'action potentielle et l'objet (point fixe). 
dans ce cas se dessine l'idée d'un répertoire d'actions potentielles, où les sensations sont des activateurs 
de ces actions.
A travers une scène fovéale visuelle, une multitudes d'actions (saccades potentielles) sont activables.
Il y a donc une carte qui associe differents points de la scène (relatifs à la position courante) à des postures
absolues. Une association (posture courante, excentricité $\rightarrow$ nouvelle posture).
